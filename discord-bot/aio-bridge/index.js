...
async function postErrorTargets(client, entry) {
  ...
  const isPlayerBug = cat === 'stuck' || cat === 'error' || cat === 'critical' || entry.pingPlayer === true;

  // Player bugs: ping the player + Rex first, then escalate to owner (same post).
  const mentionUsers = [];
  const parts = [];
  ...
  if (isPlayerBug) {
    parts.push('— player bug / stuck report. Rex escalating to owner.');
  }
  ...
  // Owner DM escalation (Rex "got the ping" → notify owner).
    ...
    dmOwner(client, {
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription(
            'Alerts post in **#general → 🚨 Rex Alerts** and ping you each time.\nPlayer bugs ping the player + Rex, then escalate to you.',
          )
          .setTimestamp(),
      ],
    }).catch(() => {});
