...
module.exports = async (client) => {
    ...
    client.loadSubcommands = async function (client, interaction, args) {
        ...
        const commandPath = `${process.cwd()}/src/commands/${interaction.commandName}/${sub}`;
        const betaPath = `${commandPath}-beta`;

        const loadModule = (filePath) => {
            try {
                console.log(`[SUBCOMMAND] Attempting to load: ${filePath}`);
                return require(filePath);
            } catch (err) {
                if (err.code === 'MODULE_NOT_FOUND') {
                    console.warn(`[SUBCOMMAND] Module not found: ${filePath}`);
                    return null;
                }
                console.error(`[SUBCOMMAND] Error loading ${filePath}:`, err.message);
                throw err;
            }
        };

        const runModule = (mod) =>
            mod(client, interaction, args).catch((err) => {
        ...
        try {
            ...
            if (data?.Beta === true) {
                const betaMod = loadModule(betaPath);
                if (betaMod) return runModule(betaMod);
            }

            const mod = loadModule(commandPath);
            if (!mod) {
                return client.errNormal({
                    error: `This command is not installed on the bot yet (${interaction.commandName} → ${sub || 'unknown'}).`,
                    type: 'editreply',
                }, interaction);
            }
