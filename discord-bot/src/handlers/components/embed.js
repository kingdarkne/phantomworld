...
module.exports = (client) => {
    ...
    client.simpleEmbed = async function ({
        ...
        if (thumbnail) embed.setThumbnail(thumbnail);
        if (fields) embed.addFields(fields);
        if (author) embed.setAuthor({ name: author[0], iconURL: author[1] });
        if (url) embed.setURL(url);
        if (color) embed.setColor(color);
        if (functiondata && functiondata.Color && !color) embed.setColor(functiondata.Color)
    ...
    client.sendEmbed = async function ({
        ...
        if (type && type.toLowerCase() == "edit") {
            ...
            // Prefer editing an already-deferred/replied interaction; never crash if channel is missing.
            try {
                ...
                console.warn('[sendEmbed] failed:', e?.message || e);
            }
            return null;
        }
    }
