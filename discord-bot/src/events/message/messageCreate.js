...
module.exports = async (client, message) => {
  ...
  if (message.channel.type === Discord.ChannelType.DM) {
    // Activity Check Response Handling
    const ActivityCheck = require("../../database/models/activityCheck");
    if (message.content.toLowerCase() === 'active') {
      const check = await ActivityCheck.findOne({ Owner: message.author.id, Status: 'Pending' });
      if (check) {
        check.Status = 'Active';
        await check.save();
        message.reply("✅ Thank you! Your server activity has been confirmed.");
        ...
        return dmlog.send({
          ...
          embeds: [
            new Discord.EmbedBuilder()
              .setTitle("✅ Activity Confirmed")
              .setDescription(`Owner ${message.author.tag} confirmed activity for guild ID: \`${check.Guild}\``)
              .setColor("#57F287")
              .setTimestamp()
          ]
        });
