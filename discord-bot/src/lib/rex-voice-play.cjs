...
async function playViaLavalink(client, { guildId, voiceChannelId, audioUrl, title }) {
  ...
  agentLog('D', 'src/lib/rex-voice-play.cjs:107', 'after voice cleanup before shoukaku join', {
    guildId,
    voiceChannelId,
  ...
  if (!player.__agentRexEventLogAttached) {
    ...
    player.on('start', (data) =>
      agentLog('C', 'src/lib/rex-voice-play.cjs:124', 'shoukaku player start event', eventData(data)),
    );
    player.on('end', (data) =>
      agentLog('C', 'src/lib/rex-voice-play.cjs:127', 'shoukaku player end event', eventData(data)),
    );
    player.on('exception', (data) =>
      agentLog('C', 'src/lib/rex-voice-play.cjs:130', 'shoukaku player exception event', eventData(data)),
    );
    player.on('stuck', (data) =>
      agentLog('C', 'src/lib/rex-voice-play.cjs:133', 'shoukaku player stuck event', eventData(data)),
    );
    player.on('closed', (data) =>
      agentLog('F', 'src/lib/rex-voice-play.cjs:136', 'shoukaku player closed event', eventData(data)),
    );
  }
  agentLog('D,F', 'src/lib/rex-voice-play.cjs:139', 'shoukaku join returned', {
    guildId,
    voiceChannelId,
    playerTrackPresent: Boolean(player.track),
  ...
  agentLog('A,C', 'src/lib/rex-voice-play.cjs:159', 'lavalink resolve selected track', {
    guildId,
  ...
  agentLog('A,C,F', 'src/lib/rex-voice-play.cjs:177', 'playTrack accepted by shoukaku', {
    guildId,
    voiceChannelId,
    playerTrackPresent: Boolean(player.track),
...
async function playRexAudio(client, { guildId, voiceChannelId, audioUrl, title }) {
  ...
  agentLog('B,D', 'src/lib/rex-voice-play.cjs:198', 'playRexAudio entry', {
    guildId,
    voiceChannelId,
    audioUrlHost: (() => {
      try {
