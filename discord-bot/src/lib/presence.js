...
/** AIO clientReady.js: DISCORD_STATUS=comma, separated, lines */
function customStatusLines() {
  const raw = process.env.DISCORD_STATUS || '';
  if (!raw.trim()) return [];
...
function buildActivities(status) {
  ...
  for (const line of customStatusLines()) {
    activities.push({
      name: trimActivity(line),
      type: ActivityType.Playing,
    });
  }
