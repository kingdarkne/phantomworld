...
async function groqTranscribe(filePath, mime = 'audio/wav') {
  ...
  // Multer temp paths have no extension — Groq requires a real audio filename.
  const base = path.basename(filePath || '');
  const hasExt = /\.(flac|mp3|mp4|mpeg|mpga|m4a|ogg|opus|wav|webm)$/i.test(base);
  const filename = hasExt ? base : 'audio.wav';
  const form = new FormData();
  form.append('file', new Blob([buf], { type: mime || 'audio/wav' }), filename);
