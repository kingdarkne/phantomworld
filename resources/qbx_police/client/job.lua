...
local function uiPrompt(promptType, id)
    if QBX.PlayerData.job.type ~= 'leo' and promptType ~= 'recruit' then return end
        ...
        while inPrompt do
            ...
            if IsControlJustReleased(0, 38) then
                if promptType == 'duty' then
                    ...
                    lib.hideTextUI()
                    break
                elseif promptType == 'recruit' then
                    applyPoliceJob()
                    lib.hideTextUI()
                    break
...
-- Recruitment zone for non-police to apply for police job
local inRecruitment = false
local function applyPoliceJob()
    TriggerServerEvent('qbx_cityhall:server:applyJob', 'police')
    lib.hideTextUI()
end
        ...
        lib.zones.box({
            ...
            onEnter = function()
                if QBX.PlayerData.job.type == 'leo' then
                    ...
                    inRecruitment = true
                    inPrompt = true
                    lib.showTextUI('[E] Apply for Police Job')
                    uiPrompt('recruit')
                end
            end,
            onExit = function()
