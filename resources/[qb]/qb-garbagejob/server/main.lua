...
QBCore.Functions.CreateCallback('qb-garbagejob:server:NewShift', function(source, cb, continue)
    ...
    if CanPay(Player) or continue then
        ...
        notify(source, Lang:t('error.not_enough', { value = Config.TruckPrice }), 'error')
    end
...
QBCore.Functions.CreateCallback('qb-garbagejob:server:NextStop', function(source, cb, currentStop, currentStopNum, currLocation)
    ...
    if (math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        ...
        if GetResourceState('ox_inventory') == 'started' and exports.ox_inventory and exports.ox_inventory.AddItem then
            local success, res = pcall(exports.ox_inventory.AddItem, source, 'cryptostick', 1)
            ok = success and res == true
        end
        if ok then
            TriggerClientEvent('ox_lib:notify', source, { title = 'Garbage Job', description = Lang:t('info.found_crypto'), type = 'success' })
        end
    end
...
RegisterNetEvent('qb-garbagejob:server:PayShift', function(continue)
    ...
    if Routes[CitizenId] ~= nil then
        ...
        if tonumber(Routes[CitizenId].stopsCompleted) < tonumber(Routes[CitizenId].totalNumberOfStops) then
            depositPay = 0
            notify(src, Lang:t('error.early_finish', { completed = Routes[CitizenId].stopsCompleted, total = Routes[CitizenId].totalNumberOfStops }), 'error')
        end
        if continue then
            depositPay = 0
        end
        ...
        notify(src, Lang:t('success.pay_slip', { total = totalToPay, deposit = payoutDeposit }), 'success')
        Routes[CitizenId] = nil
    else
        notify(source, Lang:t('error.never_clocked_on'), 'error')
    end
end)
...
QBCore.Commands.Add('cleargarbroutes', 'Removes garbo routes for user (admin only)', { { name = 'id', help = 'Player ID (may be empty)' } }, false, function(source, args)
    ...
    notify(source, Lang:t('success.clear_routes', { value = count }), 'success')
    Routes[CitizenId] = nil
end, 'admin')
