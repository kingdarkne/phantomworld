...
-- Provides only what qb-garbagejob uses.
...
local function notify(src, msg, nType)
    ...
    if IsDuplicityVersion() then
        TriggerClientEvent('ox_lib:notify', src, { title = 'Garbage Job', description = msg, type = nType or 'inform' })
    else
        lib.notify({ title = 'Garbage Job', description = msg, type = nType or 'inform' })
    end
end
