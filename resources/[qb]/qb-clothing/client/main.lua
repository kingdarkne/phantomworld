-- Variables (QBCore from client/qbx_compat.lua — qb-core export or qbx_core shim)
...
local skinData = {
    ...
    -- Allow base shirt to be fully cleared (item 0)
    ["t-shirt"] =              {item = 0,    texture = 0,  defaultItem = 0,      defaultTexture = 0},
...
local function openMenu(allowedMenus)
    ...
    local trackerMeta = metadata["tracker"]
    local translations = {}
...
local function createFirstCharacter()
    ...
    CreateThread(function()
        ...
        for _ = 1, 15 do
            ...
            if pData and pData.charinfo then break end
            Wait(200)
        end
        if not pData or not pData.charinfo then
            lib.print.error('[qb-clothing] charinfo missing — ensure qb-clothing starts after qbx_core and player is loaded.')
            return
        end
...
-- Legacy typo name (qb-clothes)
RegisterNetEvent('qb-clothes:client:CreateFirstCharacter', createFirstCharacter)
-- QBX / qbx_core and most docs use qb-clothing
RegisterNetEvent('qb-clothing:client:CreateFirstCharacter', createFirstCharacter)
...
-- Called when the clothing UI closes (first character or normal use)
RegisterNetEvent('qb-clothing:client:onMenuClose', function()
    local ped = PlayerPedId()

    -- Make sure player is not stuck or invisible after saving outfit
...
-- Safety fallback: force-close menu if NUI fails or you get stuck frozen
RegisterNetEvent('qb-clothing:client:forceCloseMenu', function()
    SetNuiFocus(false, false)
    creatingCharacter = false
    disableCam()
    FreezeEntityPosition(PlayerPedId(), false)
...
RegisterCommand('fixclothing', function()
    TriggerEvent('qb-clothing:client:forceCloseMenu')
end, false)
            ...
            clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
                if isPointInside then
                    ...
                    if zoneName == 'surgeon' then
                        QBClothingDrawText('[E] - '..Lang:t("store.surgeon"), 'left')
                    elseif zoneName == 'clothing' then
                        QBClothingDrawText('[E] - '..Lang:t("store.clothing"), 'left')
                    elseif zoneName == 'barber' then
                        QBClothingDrawText('[E] - '..Lang:t("store.barber"), 'left')
                    elseif zoneName == 'outfit' then
                        QBClothingDrawText('[E] - '..Lang:t("store.outfitchanger"), 'left')
                    end
                else
                    inZone = false
                    QBClothingHideText()
