...
--- Fuel script export (e.g. cdn-fuel); falls back to natives if resource missing or export unavailable.
local function GetVehicleFuelSafe(veh)
    local res = Config.FuelResource
    if res and GetResourceState(res) == 'started' then
        local ok, level = pcall(function()
            return exports[res]:GetFuel(veh)
        end)
...
local function SetVehicleFuelSafe(veh, fuelLevel)
    local res = Config.FuelResource
    if res and GetResourceState(res) == 'started' then
        local ok = pcall(function()
            exports[res]:SetFuel(veh, fuelLevel)
        end)
...
local function OpenGarageMenu(data)
    QBCore.Functions.TriggerCallback('qb-garages:server:GetGarageVehicles', function(result)
        ...
        if #result == 0 then
            QBCore.Functions.Notify('You don\'t have any vehicles stored here yet. Go do some jobs or rob stores to earn money and buy a car!', 'error', 8000)
        end
...
local function DepositVehicle(veh, data)
    ...
    QBCore.Functions.TriggerCallback('qb-garages:server:canDeposit', function(canDeposit)
        if canDeposit then
            ...
            local totalFuel = GetVehicleFuelSafe(veh)
...
local function FindNearestGarage(filterFn)
    ...
    for index, garage in pairs(Config.Garages) do
        if garage and garage.takeVehicle and (not filterFn or filterFn(index, garage)) then
            local gcoords = garage.takeVehicle
...
RegisterNetEvent('qb-garages:client:openNearestPublicGarage', function()
    local index = FindNearestGarage(function(_, garage)
        return garage.type == 'public'
    end)

    if not index or not Config.Garages[index] then
        QBCore.Functions.Notify('No public garage found.', 'error', 3500)
        return
    end

    OpenGarageMenu({
        indexgarage = index,
        type = Config.Garages[index].type,
        category = Config.Garages[index].category,
    })
end)
    ...
    QBCore.Functions.TriggerCallback('qb-garages:server:IsSpawnOk', function(spawn)
        if spawn then
            ...
            QBCore.Functions.TriggerCallback('qb-garages:server:spawnvehicle', function(netId, properties, vehPlate)
                ...
                SetVehicleFuelSafe(veh, data.stats.fuel)
