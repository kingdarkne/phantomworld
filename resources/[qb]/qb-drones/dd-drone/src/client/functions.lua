...
function GetGround(pos)
  ...
  if not found then
      return vector3(pos.x, pos.y, pos.z - 1.0)  -- Return 1 meter below if ground not found
  else
      return vector3(pos.x, pos.y, z)  -- Return ground z coordinate
  end
end
...
function GetGroundZ(pos)
  local found, z = GetGroundZFor_3dCoord(pos.x, pos.y, pos.z, false)
  if not found then
      return GetGround(vector3(pos.x, pos.y, pos.z - 1.0))  -- Call GetGround if ground not found
  else
...
function CreateBlip(pos,sprite,color,text,scale,display,shortRange,highDetail)
  ...
  AddTextComponentString(text and text or ("Blip " .. tostring(blip)))
