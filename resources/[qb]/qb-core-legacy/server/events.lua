...
RegisterNetEvent('QBCore:CallCommand', function(command, args)
    ...
    if hasPerm then
        if QBCore.Commands.List[command].argsrequired and #QBCore.Commands.List[command].arguments ~= 0 and not args[#QBCore.Commands.List[command].arguments] then
            ...
            local cb = QBCore.Commands.List[command].callback
...
-- Also auto-give keys for the spawned vehicle to the spawning player via qb-vehiclekeys.
QBCore.Functions.CreateCallback('QBCore:Server:SpawnVehicle', function(source, cb, model, coords, warp)
    ...
    if veh and veh ~= 0 then
        ...
        if plate and plate ~= '' then
            local trimmed = QBCore.Shared.Trim(plate)
            if GetResourceState('qb-vehiclekeys') == 'started' then
                exports['qb-vehiclekeys']:GiveKeys(source, trimmed)
            end
        end
    end
end)
