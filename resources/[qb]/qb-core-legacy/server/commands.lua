...
function QBCore.Commands.Add(name, help, arguments, argsrequired, callback, permission, ...)
    ...
    -- Support alternate order: (name, help, arguments, callback, argsrequired, permission) when 5th is not a function
    if type(callback) ~= 'function' and type(argsrequired) == 'function' then
        local orig = callback
        callback = argsrequired
        argsrequired = orig
    end
    -- Support (name, help, callback, arguments, argsrequired, permission): 3rd param is callback when it's a function and 5th is not
    if type(callback) ~= 'function' and type(arguments) == 'function' then
        local fifth = callback
        callback = arguments
        arguments = argsrequired
        argsrequired = fifth
    end
    -- Don't overwrite a valid command with an invalid one (e.g. another resource wrong param order -> callback is table)
    local existing = QBCore.Commands.List[key]
    if existing and type(existing.callback) == 'function' and type(callback) ~= 'function' then
        print(('[qb-core] Ignoring invalid re-registration of command "%s" (callback is %s); keeping existing.'):format(name, type(callback)))
        return
    end
    ...
    RegisterCommand(name, function(source, args, rawCommand) -- Register command within fivem
        if argsrequired and arguments and #args < #arguments then
        ...
        if type(callback) == 'function' then
            ...
            print(('[qb-core] Command "%s" has no valid callback (ref is %s)'):format(name, type(callback)))
        end
    end, restricted)
...
QBCore.Commands.Add('car', Lang:t('command.car.help'), { { name = Lang:t('command.car.params.model.name'), help = Lang:t('command.car.params.model.help') } }, true, function(source, args)
    ...
    -- Membership hook: some high-end/addon cars can be restricted to supporters.
    -- dr-membership export returns true if no restriction or player meets required tier.
    if GetResourceState('dr-membership') == 'started' then
        local ok = exports['dr-membership']:HasVehicleAccess(source, model)
        if not ok then
            TriggerClientEvent('QBCore:Notify', source, 'You need a higher membership tier for this vehicle.', 'error')
            return
        end
    end

    TriggerClientEvent('QBCore:Command:SpawnVehicle', source, model)
end, 'admin')
...
-- Money: RegisterCommand only so no "callback is table" from Commands.Add param order
CreateThread(function()
    Wait(500)
    local givemoneyHandler = function(source, args)
        args = args or {}
    ...
    local setmoneyHandler = function(source, args)
        args = args or {}
        if not QBCore.Functions.HasPermission(source, 'admin') then
            TriggerClientEvent('QBCore:Notify', source, 'You do not have permission to use this command.', 'error')
            return
    ...
    RegisterCommand('givemoney', givemoneyHandler, false)
    RegisterCommand('setmoney', setmoneyHandler, false)
end)
...
-- setjob: RegisterCommand only = no "callback is table"
CreateThread(function()
    Wait(500)
    local setjobHandler = function(source, args)
        args = args or {}
        if not QBCore.Functions.HasPermission(source, 'admin') then
            TriggerClientEvent('QBCore:Notify', source, 'You do not have permission to use this command.', 'error')
            return
    ...
    RegisterCommand('setjob', setjobHandler, false)
end)
