...
RegisterNetEvent('QBCore:Command:SpawnVehicle', function(vehName)
    local ped = PlayerPedId()
    -- Allow spawning by number: /car 1, /car 2, etc. (order from shared/vehicles.lua)
    local model = vehName
    local num = tonumber(vehName)
    if num and QBCore.Shared and QBCore.Shared.VehicleByIndex and QBCore.Shared.VehicleByIndex[num] then
        model = QBCore.Shared.VehicleByIndex[num]
    end
    local hash = joaat(model)
    local veh = GetVehiclePedIsUsing(ped)
    if not IsModelInCdimage(hash) then
        QBCore.Functions.Notify('Invalid vehicle: ' .. tostring(vehName), 'error')
        return
    end
    RequestModel(hash)
...
-- Show vehicle list by number (for /car 1, /car 2, etc.). Usage: /vehiclelist [page]
RegisterCommand('vehiclelist', function(_, args)
    local shared = QBCore.Shared
    if not shared or not shared.VehicleByIndex or not shared.Vehicles then return end
    local perPage = 20
    local total = shared.VehicleCount or 0
    local page = math.max(1, tonumber(args[1]) or 1)
    local start = (page - 1) * perPage + 1
    local last = math.min(start + perPage - 1, total)
    if start > total then
        QBCore.Functions.Notify('No vehicles on page ' .. page .. '. Total: ' .. total, 'error')
        return
    end
    for i = start, last do
        local model = shared.VehicleByIndex[i]
        local v = shared.Vehicles[model]
        local name = v and v.name or model
        QBCore.Functions.Notify(('%s = %s (%s)'):format(i, model, name), 'primary', 8000)
    end
    QBCore.Functions.Notify(('Page %s: vehicles %s–%s of %s. Use /vehiclelist %s for next.'):format(page, start, last, total, page + 1), 'success', 6000)
end, false)
