return {
    ...
    -- IMPORTANT: Only include tables that actually exist in your database,
    -- otherwise Qbox will log warnings on startup.
    characterDataTables = {
        {'players', 'citizenid'},
        ...
        -- Note: bank_accounts_new has 'name' not citizenid so it is
        -- managed separately by job/society logic, not per-character delete.
        -- Add any other custom tables that should be cleaned up on character delete, e.g.:
        -- {'your_custom_table', 'citizenid'},
    ...
    server = {
        ...
        -- When true, admins must run /optin before /givemoney, /car, etc. Set false on small/private servers if you only use ACE `group.admin`.
        requireOptIn = false,
    ...
    -- Society account helpers (backed by Qbox's bank_accounts_new table)
    getSocietyAccount = function(accountName)
        local balance = MySQL.scalar.await('SELECT balance FROM bank_accounts_new WHERE name = ?', { accountName })
        return balance or 0
    end,

    removeSocietyMoney = function(accountName, payment)
        local affected = MySQL.update.await(
            'UPDATE bank_accounts_new SET balance = balance - ? WHERE name = ? AND balance >= ?',
