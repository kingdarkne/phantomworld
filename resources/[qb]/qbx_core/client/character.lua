...
--- Never use :find('start') on GetResourceState — state "starting" also matches and breaks appearance order.
---@param name string
---@return boolean
...
local function openCharacterCreator()
    ...
    CreateThread(function()
        Wait(500)
        if resStarted('illenium-appearance') then
            -- illenium-appearance (QB) listens for this event name, not illenium-appearance:client:openCharacterCreator
            TriggerEvent('qb-clothes:client:CreateFirstCharacter')
            return
        end
        if resStarted('fivem-appearance') then
            TriggerEvent('fivem-appearance:client:openCreator')
            return
        end
        if resStarted('qb-clothing') then
            TriggerEvent('qb-clothing:client:CreateFirstCharacter')
            return
        end
        lib.print.error('[qbx_core] No appearance resource running. Start one of: illenium-appearance, fivem-appearance, or qb-clothing (after qbx_core).')
    end)
end
...
local function spawnDefault() -- We use a callback to make the server wait on this to be done
    ...
    local fadeDeadline = GetGameTimer() + 15000
    while not IsScreenFadedIn() do
        if GetGameTimer() > fadeDeadline then
            DoScreenFadeIn(500)
            Wait(500)
            break
        end
...
--- Match legacy dr-multichar + renzu_spawn: after renzu selector finishes, fire the same client/server hooks as spawnLastLocation.
local function finalizeSpawnAfterRenzu()
    TriggerServerEvent('QBCore:Server:OnPlayerLoaded')
    TriggerEvent('QBCore:Client:OnPlayerLoaded')
    TriggerServerEvent('qb-houses:server:SetInsideMeta', 0, false)
    TriggerServerEvent('qb-apartments:server:SetInsideMeta', 0, 0, false)
    if resStarted('dr-weathersync') then
...
local function spawnLastLocation()
    ...
    local fadeDeadline2 = GetGameTimer() + 15000
    while not IsScreenFadedIn() do
        if GetGameTimer() > fadeDeadline2 then
            DoScreenFadeIn(500)
            Wait(500)
            break
        end
...
local function createCharacter(cid)
    ...
    if resStarted('renzu_spawn') then
        destroyPreviewCam()
        exports.renzu_spawn:Selector()
        finalizeSpawnAfterRenzu()
        openCharacterCreator()
