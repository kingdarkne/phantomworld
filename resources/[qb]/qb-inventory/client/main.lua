...
local function HasItem(items, amount)
    ...
    if PlayerData.items == nil then return false end
...
RegisterNetEvent('inventory:client:OpenInventory', function(PlayerAmmo, inventory, other)
    ...
    if not IsEntityDead(PlayerPedId()) then
        ...
        if Config.OrApartment == true then
            rooms = exports["dr-apartment"]:GetPlayerOwnedRooms()
            if next(rooms) then
                room = rooms[#rooms]
                room.apartment_label = exports["dr-apartment"]:GetApartmentLabelById(room.apartment_id)
            end
        end
...
RegisterNetEvent('inventory:client:UseWeapon', function(weaponData, shootbool)
    ...
    -- Enforce firearm license only for high-grade weapons (rifles/SMGs/shotguns/snipers/heavies)
    local HighGradeWeapons = {
        -- SMGs
    ...
    if HighGradeWeapons[weaponName] then
        local hasLicense = QBCore.Functions.HasItem('weaponlicense')
        if not hasLicense then
            QBCore.Functions.Notify('You need a valid weapon license to use high-grade weapons.', 'error')
            return
        end
    end

    -- Rank lock: block guns above the player's XNL rank (only if dr-rankxp is running)
    local ok, allowed, required, current = pcall(function()
        local can, req, lvl = exports['dr-rankxp']:CanUseWeapon(weaponName)
        return can, req, lvl
    end)
    if ok and allowed ~= nil and not allowed then
        QBCore.Functions.Notify(('You need rank %s to use this weapon. You are rank %s.'):format(required, current), 'error')
        return
    end
