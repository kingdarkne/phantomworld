...
$(document).on('click', '.bank-app-account', function(e){
    ...
    QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "Account number copied!", "#badc58", 1750);
});
...
$(document).on('click', '.bank-app-header-button', function(e){
    ...
    var PressedTab = $(PressedObject).attr('data-headertype') || $(PressedObject).data('headertype') || "accounts";
...
QB.Phone.Functions.DoBankOpen = function() {
    $(".bank-app-loaded").css({"display":"none"});
    $(".bank-app-accounts").css({"left":"30vh"});
    $("#qbank-text").css({"opacity":"0"});
    $(".bank-app-loading").css({"display":"block", "left":"0"});
    setTimeout(function(){
        CurrentTab = "accounts";
        setTimeout(function(){
            $("#qbank-text").animate({ opacity: 1 }, 400);
        }, 100);
        setTimeout(function(){
            $(".bank-app-loaded").css({"display":"block"});
            $(".bank-app-accounts").animate({left: "0"}, 300);
            $(".bank-app-loading").animate({ opacity: 0 }, 250, function(){
                $(".bank-app-loading").css({"display":"none", "left":"0"});
            });
        }, 1200);
    }, 400);
}
...
QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "You have transferred &#36; "+amount+"!", "#badc58", 1500);
                    } else {
                    QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "You don't have enough balance!", "#e74c3c", 1500);
                }
                QB.Phone.Animations.TopSlideUp(".bank-app-transfer", 400, -100);
            });
    } else {
        QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "Fill out all fields!", "#f39c12", 1750);
    }
});
    ...
    if (BankBalance >= InvoiceData.amount) {
        $.post('https://qb-phone/PayInvoice', JSON.stringify({
            ...
            if (CanPay) {
                ...
                QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "You have paid &#36;"+InvoiceData.amount+"!", "#badc58", 1500);
                var amountData = $(".bank-app-account-balance").data('balance');
...
$(document).on('click', '.bank-app-my-contact', function(e){
    ...
    if (PressedContactData.iban !== "" && PressedContactData.iban !== undefined && PressedContactData.iban !== null) {
        ...
        QB.Phone.Notifications.Add("fas fa-university", "Welcome Phantom", "No bank account attached to this contact.", "#f39c12", 2500);
    }
    QB.Phone.Animations.TopSlideUp(".bank-app-my-contacts", 400, -100);
});