...
print("^2[QB-Phone] ^7Premium QB-Phone by FiveM QBCore")
print("^2[QB-Phone] ^7For premium server buy visit here:")
print("^2[QB-Phone] ^7🌍 Website: https://fivem-qbcore.com")
print("^2[QB-Phone] ^7💬 Discord: https://discord.gg/qbcoreframework")
...
local function OpenPhone()
    if (Config.AllowOpenWithoutPhone ~= false) or hasPhone() then
        PhoneData.PlayerData = PlayerData
...
RegisterCommand('phone', function()
    if not PhoneData.isOpen then
        ...
        if not meta['ishandcuffed'] and not meta['inlaststand'] and not meta['isdead'] and not IsPauseMenuActive() then
            OpenPhone()
        else
...
-- Open OMES banking UI from phone (mobile bank app – same as being at a bank)
RegisterNUICallback('OpenMobileBank', function(_, cb)
    cb('ok')
    ...
    SetTimeout(400, function()
        ...
        SetTimeout(100, function()
            TriggerServerEvent('omes_banking:openUI')
        end)
    end)
    SetTimeout(300, function()
        SetNuiFocus(false, false)
...
-- app sory install or unstall app document 
-- install function
RegisterNUICallback('AddNewInstallApp', function(data, cb)
    local AppName = data.CurrentAppName
    local AppNumber = data.CurrentAppNumber
    TriggerServerEvent('qb-phone:server:AddNewInstallApp', AppName, AppNumber)
    cb(PhoneData.AppData)
end)

RegisterNUICallback('AddOldApp', function(data, cb)
    cb(PhoneData.AppData)
end)

RegisterNUICallback('RemoveUnisatallApp', function(data, cb)
    local AppName = data.CurrentAppName
    local AppNumber = data.CurrentAppNumber

    TriggerServerEvent('qb-phone:server:RemoveUnisatallApp', AppName, AppNumber)
    cb(PhoneData.AppData)
end)

-- CreateThread(function()
--     Wait(2000)
--     TriggerServerEvent('qb-phone:server:loadPlayerAppstoreData')
-- end)
...
RegisterNetEvent("qb-phone:server:loadPlayerAppstoreData", function(result)
    if result ~= nil then
        for i,v in pairs(result) do
            if type(v) == "table" then
                SendNUIMessage({
                    appname = v.appname,
                    appnumber = v.appnumber
                })
            end
        end
    end
