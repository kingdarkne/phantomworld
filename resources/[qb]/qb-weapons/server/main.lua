...
--- qbx_core + ox_inventory: `Player.Functions.SetInventory` is not supported. Persist slot edits via SetMetadata.
local function mergeItemMetadata(item)
    local meta = {}
    if item.metadata then
        for k, v in pairs(item.metadata) do meta[k] = v end
    end
    if item.info then
        for k, v in pairs(item.info) do meta[k] = v end
    end
    return meta
end
...
---@param Player table
---@param slot number
local function syncOxInventorySlot(Player, slot)
    if not Player or not slot then return end
    local item = Player.PlayerData.items and Player.PlayerData.items[slot]
    if not item then return end
    exports.ox_inventory:SetMetadata(Player.PlayerData.source, slot, mergeItemMetadata(item))
end
...
QBCore.Functions.CreateCallback('weapons:server:RemoveAttachment', function(source, cb, AttachmentData, ItemData)
    ...
    if Inventory[ItemData.slot] then
        Inventory[ItemData.slot].info = Inventory[ItemData.slot].info or Inventory[ItemData.slot].metadata or {}
        if Inventory[ItemData.slot].info.attachments and next(Inventory[ItemData.slot].info.attachments) then
            ...
            if HasAttach then
                for k, v in pairs(Inventory[ItemData.slot].info.attachments) do
                    table.remove(Inventory[ItemData.slot].info.attachments, key)
                    syncOxInventorySlot(Player, ItemData.slot)
                    Player.Functions.AddItem(AttachmentComponent.item, 1, false, v)
...
RegisterNetEvent("weapons:server:UpdateWeaponAmmo", function(CurrentWeaponData, amount)
    ...
    if CurrentWeaponData then
        if Player.PlayerData.items[CurrentWeaponData.slot] then
            Player.PlayerData.items[CurrentWeaponData.slot].info = Player.PlayerData.items[CurrentWeaponData.slot].info
                or Player.PlayerData.items[CurrentWeaponData.slot].metadata
                or {}
            Player.PlayerData.items[CurrentWeaponData.slot].info.ammo = amount
        end
        syncOxInventorySlot(Player, CurrentWeaponData.slot)
    end
end)
...
RegisterNetEvent("weapons:server:SetWeaponQuality", function(data, hp)
    ...
    WeaponSlot.info = WeaponSlot.info or WeaponSlot.metadata or {}
    WeaponSlot.info.quality = hp
    syncOxInventorySlot(Player, data.slot)
end)
...
RegisterNetEvent("weapons:server:EquipAttachment", function(ItemData, CurrentWeaponData, AttachmentData)
    ...
    if Inventory[CurrentWeaponData.slot] then
        Inventory[CurrentWeaponData.slot].info = Inventory[CurrentWeaponData.slot].info or Inventory[CurrentWeaponData.slot].metadata or {}
        if Inventory[ItemData.slot] then
            Inventory[ItemData.slot].info = Inventory[ItemData.slot].info or Inventory[ItemData.slot].metadata or {}
        end
        if Inventory[CurrentWeaponData.slot].info.attachments and next(Inventory[CurrentWeaponData.slot].info.attachments) then
            ...
            if not HasAttach then
                ...
                Inventory[CurrentWeaponData.slot].info.attachments[#Inventory[CurrentWeaponData.slot].info.attachments+1] = {
                    ...
                    quality = Inventory[ItemData.slot].info.quality
                }
