...
function addNumber(num) {
    var input = document.getElementById("phone-number-call-free"); 
    if (input && input.value.length < 14) { 
        input.value += num;
    }
}
                ...
                if (status.IsOnline) {
                    if (status.CanCall) {
                        if (!status.InCall) {
                            ...
                            ClearInputNew()
                            clearInputField(InputNum); // Clear the input field
                            $(".phone-call-outgoing").css({"display":"none"});
                            $(".phone-call-incoming").css({"display":"none"});
                            $(".phone-call-ongoing").css({"display":"none"});
                            $(".phone-call-outgoing-caller").html(cData.name);
...
function clearInputField(InputNum) {
    InputNum.val(''); 
}

// product view or show======================================================
$(document).ready(function(){
...
QB.Phone.Functions.LoadContacts = function(myContacts) {
    ...
    if (myContacts !== null) {
        $.each(myContacts, function(i, contact){
            ...
            var ContactElement = '<div class="phone-contact" data-contactid="'+i+'"><div class="phone-contact-name">'+contact.name+'</div><div class="phone-contact-action-buttons"> <i class="fas fa-user-alt-slash" id="delete-contact" data-toggle="tooltip" title="Delete"></i> <i class="fas fa-phone" id="phone-start-call" data-toggle="tooltip" title="Call"></i> <i class="fas fa-comment" id="new-chat-phone" data-toggle="tooltip" title="Message"></i> <i class="fas fa-edit" id="edit-contact" data-toggle="tooltip" title="Edit"></i> <i class="fas fa-clipboard" id="copy-contact" data-toggle="tooltip" title="Copy"></i>  </div></div>'
...
$(document).on('click', '#phone-number-savecontact', function(e){
    ...
    var ContactName = DOMPurify.sanitize($(".phone-number-call-name").val() , {ALLOWED_TAGS: [],ALLOWED_ATTR: []});
    if (ContactName == '') ContactName = 'Hmm, I shouldn\'t be able to do this...'
