...
-- server\main.lua included appinstall system install or unstall
RegisterNetEvent('qb-phone:server:AddNewInstallApp', function(appname, appnumber)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if not Player then return end

    exports.oxmysql:insert('INSERT INTO app_store (citizenid, appname, appnumber) VALUES (?, ?, ?)',
    {Player.PlayerData.citizenid, tostring(appname), appnumber})
end)

RegisterNetEvent('qb-phone:server:RemoveUnisatallApp', function(appname, appnumber)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)

    if not Player then return end

    exports.oxmysql:execute('DELETE FROM app_store WHERE appname = ? AND appnumber = ? AND citizenid = ?',
    {appname, appnumber, Player.PlayerData.citizenid})
end)

RegisterNetEvent("qb-phone:server:loadPlayerAppstoreData", function()
    local src = source
    local player = QBCore.Functions.GetPlayer(src)
    local cid = player.PlayerData.citizenid
    ...
    local data = exports.oxmysql:executeSync('SELECT * FROM app_store WHERE citizenid = ?', {cid})
	TriggerClientEvent("qb-phone:server:loadPlayerAppstoreData", src, data)
end)