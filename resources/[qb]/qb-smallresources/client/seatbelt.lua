...
-- Loud recurring reminder while driver, in a car, unbuckled (no harness)
CreateThread(function()
    while true do
        local cfg = Config.SeatbeltReminder
        local interval = (cfg and cfg.intervalMs) or 900
        Wait(interval)
        if not cfg or not cfg.enabled then goto continue end
        if IsPauseMenuActive() then goto continue end
        local ped = PlayerPedId()
        ...
        if class == 8 or class == 13 or class == 14 then goto continue end -- bike / cycle / boat
        if IsThisModelABike(GetEntityModel(veh)) then goto continue end
        if seatbeltOn or harnessOn then goto continue end
        local msg = cfg.message or 'PUT YOUR SEATBELT ON NOW!!!!!!!!!!!!!!!'
        QBCore.Functions.Notify(msg, 'error', math.min(interval - 50, 8000))
        ::continue::
    end
end)