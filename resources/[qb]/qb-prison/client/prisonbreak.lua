...
local function itemUiEntry(itemName)
    local items = QBCore and QBCore.Shared and QBCore.Shared.Items
    local def = items and items[itemName]
    if def then
        return { name = def.label or def.name or itemName, image = def.image or ('nui://ox_inventory/web/images/' .. itemName .. '.png') }
    end
    return { name = itemName, image = 'nui://ox_inventory/web/images/' .. itemName .. '.png' }
end

CreateThread(function()
    Wait(500)
    requiredItems = {
        [1] = itemUiEntry('electronickit'),
        [2] = itemUiEntry('gatecrack'),
    }
    while true do
        inRange = false
        currentGate = 0
