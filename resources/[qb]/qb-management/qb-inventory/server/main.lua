...
-- giveitem: givexp-style deferred reg + permission in handler + fallback
local function registerGiveItemCommand()
	local handler = function(source, args)
		args = args or {}
		...
		if not args[1] or not args[2] then
			TriggerClientEvent('QBCore:Notify', source, 'Use: /giveitem <player_id> <item_name> [amount]', 'error')
			return
		end
		local id = tonumber(args[1])
		local Player = QBCore.Functions.GetPlayer(id)
		local amount = tonumber(args[3]) or 1
		local itemName = args[2] and tostring(args[2]):lower() or nil
		local itemData = itemName and QBCore.Shared.Items[itemName] or nil
		if Player then
			if itemData then
				-- check iteminfo
				local info = {}
	...
	RegisterCommand("giveitem", handler, false)
end
CreateThread(function() Wait(500) registerGiveItemCommand() end)
