...
-- Cache: which plates are \"owned\" (tracked in server VehicleList) so we only enforce hotwire on real owned cars
local OwnedPlates = {}

local function IsPlateOwned(plate, cb)
    if not plate or plate == '' then
        cb(false)
        return
    end
    if OwnedPlates[plate] ~= nil then
        cb(OwnedPlates[plate])
        return
    end
    QBCore.Functions.TriggerCallback('qb-vehiclekeys:server:checkPlayerOwned', function(playerOwned)
        OwnedPlates[plate] = playerOwned and true or false
        cb(OwnedPlates[plate])
    end, plate)
end
    ...
    if looped == false then
        ...
        while true do
            ...
            if LocalPlayer.state.isLoggedIn then
                ...
                -- Hotwiring while in vehicle, but only for player-owned cars.
                -- Street/NPC cars that are NOT registered as owned will NOT require hotwire at all.
                if IsPedInAnyVehicle(ped, false) and not IsHotwiring then
                    ...
                    if GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() and not HasKeys(plate) and not isBlacklistedVehicle(vehicle) and not AreKeysJobShared(vehicle) then
                        IsPlateOwned(plate, function(playerOwned)
                            -- If this plate is not tracked/owned, just let the car run normally
                            if not playerOwned then
...
RegisterNetEvent('lockpicks:UseLockpick', function(isAdvanced)
    ...
    -- cure-lockpick OpenLockpickMinigame(successCallback, target, time) - time is optional number
