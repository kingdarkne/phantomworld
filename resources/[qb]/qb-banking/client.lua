...
-- Open full bank UI from phone (mobile bank app) – same as being at a bank
RegisterNetEvent('qb-banking:client:openBankFromPhone', function()
    OpenBank()
end)
...
-- ATM blips (if atmLocations exists)
CreateThread(function()
    if not Config.atmLocations then return end
    for i = 1, #Config.atmLocations do
        local blip = AddBlipForCoord(Config.atmLocations[i])
        SetBlipSprite(blip, Config.atmBlipInfo and Config.atmBlipInfo.sprite or 277)
        SetBlipDisplay(blip, 4)
        SetBlipScale(blip, Config.atmBlipInfo and Config.atmBlipInfo.scale or 0.5)
        SetBlipColour(blip, Config.atmBlipInfo and Config.atmBlipInfo.color or 2)
        SetBlipAsShortRange(blip, true)
        BeginTextCommandSetBlipName('STRING')
        AddTextComponentSubstringPlayerName(tostring(Config.atmBlipInfo and Config.atmBlipInfo.name or 'ATM'))
        EndTextCommandSetBlipName(blip)
    end
end)
    ...
    CreateThread(function()
        ...
        combo:onPlayerInOut(function(isPointInside)
            ...
            if isPlayerInsideBankZone then
                exports['qb-core']:DrawText('Phantom Banking')
                CreateThread(function()
