...
-- Dealership preview camera: created once when entering, destroyed when leaving
...
local function updatePreviewCam()
    if Config.DisablePreviewCam or not insideShop or not ClosestVehicle or not Config.Shops[insideShop] then return end
    local slot = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle]
    if not slot or not slot.coords then return end

    local pos = vector3(slot.coords.x, slot.coords.y, slot.coords.z)
    local heading = slot.coords.w or 0.0
    local backDist = 5.0
    local rad = math.rad(heading)
    local camX = pos.x - math.cos(rad) * backDist
    local camY = pos.y - math.sin(rad) * backDist
    local camZ = pos.z + 1.5

    if not previewCam or not DoesCamExist(previewCam) then
        previewCam = CreateCam("DEFAULT_SCRIPTED_CAMERA", false)
        SetCamFov(previewCam, 55.0)
        SetCamCoord(previewCam, camX, camY, camZ)
        PointCamAtCoord(previewCam, pos.x, pos.y, pos.z + 0.5)
...
-- Keeps script cam active while in dealer so the game doesn't override it
local function startPreviewCamKeeper()
    if Config.DisablePreviewCam then return end
    CreateThread(function()
        while insideShop do
            if previewCam and DoesCamExist(previewCam) then
                if not IsCamActive(previewCam) then
                    SetCamActive(previewCam, true)
...
-- Match server-side multipliers so UI shows the real price
local CategoryPriceMultiplier = {
    compacts       = 1.0,
    sedans         = 1.0,
    coupes         = 1.1,
    suv            = 1.2,
...
local function getVehPriceRaw(model)
    local data = QBCore.Shared.Vehicles[model]
    if not data then return 0 end
    local base = tonumber(data.price) or 0
    local mult = CategoryPriceMultiplier[data.category] or 1.0
    return math.floor(base * mult + 0.5)
end

local function getVehPrice()
    local model = Config.Shops[insideShop]['ShowroomVehicles'][ClosestVehicle].chosenVehicle
    return comma_value(getVehPriceRaw(model))
end
...
local function setClosestShowroomVehicle()
    ...
    if current ~= ClosestVehicle then
        ...
        if not Config.DisablePreviewCam then updatePreviewCam() end
    end
end
...
local function createFreeUseShop(shopShape, name)
    ...
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            insideShop = name
            CreateThread(function()
                if not Config.DisablePreviewCam then
                    startPreviewCamKeeper()
                    Wait(400)
                    updatePreviewCam()
                end
                while insideShop do
...
RegisterNetEvent('qb-vehicleshop:client:swapVehicle', function(data)
    ...
    if Config.Shops[shopName]['ShowroomVehicles'][data.ClosestVehicle].chosenVehicle ~= data.toVehicle then
        ...
        if insideShop == shopName and not Config.DisablePreviewCam then
            updatePreviewCam()
        end
    end
end)
