...
local function setupClient()
    ...
    if playerJob.name == Config.Jobname then
        ...
        SetBlipSprite(securityBlip, 487)
        SetBlipDisplay(securityBlip, 4)
        SetBlipScale(securityBlip, 0.8)
        SetBlipAsShortRange(securityBlip, true)
        SetBlipColour(securityBlip, 2)
                ...
                if not hasBag and canTakeBag then
                    if Distance < 1.5 then
                        if not GarbText then
                            GarbText = true
                            QBCore.Functions.DrawText(Lang:t("info.grab_cashpickup"), 'left')
                        end
                    ...
                    if DoesEntityExist(securityVehicle) then
                        ...
                        if TruckDist < 6 then
                            if not TrucText then
                                TrucText = true
                                QBCore.Functions.DrawText(Lang:t("info.dispose_cashpickup"), 'left')
                            end
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['qb-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "qb-securityjob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-sack-dollar', job = "security",}},
                distance = 2.0
            })
        else
            ...
            if options then
                ...
                zone:onPlayerInOut(function(inside)
                    if LocalPlayer.state.isLoggedIn then
                        if inside then
                            QBCore.Functions.DrawText(Lang:t("info.talk"), 'left')
                            Listen4Control()
                        else
                            ControlListen = false
                            QBCore.Functions.HideText()
