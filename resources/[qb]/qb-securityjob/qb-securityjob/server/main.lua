local QBCore = QBCore or { Functions = {}, Commands = {} }
...
QBCore.Functions.CreateCallback("securityjob:server:NewShift", function(source, cb, continue)
    ...
    if CanPay(Player) or continue then
        ...
        local MaxStops = math.random(Config.MinStops, #Config.Locations["cashbox"])
        local allStops = {}

        for _=1, MaxStops do
            local stop = math.random(#Config.Locations["cashbox"])
...
RegisterNetEvent("qb-securityjob:server:payDeposit", function()
    local Player = QBCore.Functions.GetPlayer(source)
    if not Player.Functions.RemoveMoney("bank", Config.TruckPrice, "security-deposit") then
        TriggerClientEvent('QBCore:Notify', source, Lang:t("error.not_enough", {value = Config.TruckPrice}), "error")
    end
end)

QBCore.Functions.CreateCallback("securityjob:server:NextStop", function(source, cb, currentStop, currentStopNum, currLocation)
    local Player = QBCore.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid

    local currStopCoords = Config.Locations["cashbox"][currentStop].coords
    ...
    if(math.random(100) >= Config.CryptoStickChance) and Config.GiveCryptoStick then
        if GetResourceState('ox_inventory') == 'started' and exports.ox_inventory and exports.ox_inventory.AddItem then
            pcall(exports.ox_inventory.AddItem, source, 'cryptostick', 1)
        else
            Player.Functions.AddItem("cryptostick", 1, false)
        end
        TriggerClientEvent('ox_lib:notify', source, { title = 'Security Job', description = Lang:t("info.found_crypto"), type = 'success' })
    end
...
QBCore.Functions.CreateCallback('securityjob:server:EndShift', function(source, cb)
    local Player = QBCore.Functions.GetPlayer(source)
    local CitizenId = Player.PlayerData.citizenid
    local status = false
...
RegisterNetEvent('securityjob:server:PayShift', function(continue)
    local src = source
    ...
    if Routes[CitizenId] ~= nil then
        ...
        Player.Functions.AddMoney("bank", totalToPay , 'security-payslip')
