...
// Phantom top-right strip (server name, time, ID, rank, job, voice)
const phantomTop = Vue.createApp({
  data() {
    return {
      visible: false,
      serverName: '',
  ...
  mounted() {
    window.addEventListener('message', (e) => {
      const d = e.data;
      if (d.action === 'phantomInfo') {
        if (d.visible !== undefined) this.visible = !!d.visible;
        if (d.serverName !== undefined) this.serverName = d.serverName;
        if (d.serverTime !== undefined) this.serverTime = d.serverTime;
        if (d.playerId !== undefined) this.playerId = d.playerId;
        if (d.playerName !== undefined) this.playerName = d.playerName;
        if (d.gang !== undefined) this.gang = d.gang;
        if (d.playerStreet !== undefined) this.street = d.playerStreet;
        if (d.rank !== undefined) this.rank = d.rank;
        if (d.job !== undefined) this.job = d.job;
        if (d.showRank !== undefined) this.showRank = d.showRank;
        if (d.showJob !== undefined) this.showJob = d.showJob;
        if (d.cash !== undefined) this.cash = d.cash;
        if (d.bank !== undefined) this.bank = d.bank;
      }
      if (d.action === 'phantomVoice') {
        this.talking = !!d.talking;
        if (d.voiceLabel) this.voiceLabel = d.voiceLabel;
      }
      if (d.action === "topRightStack") {
        const el = document.getElementById("top-right-stack");
        if (el) {
  ...
  template: `
    <div v-show="visible" class="phantom-strip-wrap">
      <div class="phantom-strip-glass">
        <div class="phantom-head">
          <span class="phantom-server-name">{{ serverName || 'Server' }}</span>
          <div class="phantom-chips">
            <span class="phantom-chip phantom-time">{{ serverTime || '—' }}</span>
            <span class="phantom-chip phantom-id">ID {{ playerId }}</span>
          </div>
        </div>
        <div class="phantom-line2">{{ playerName }} · {{ gang }} · {{ street }}</div>
        <div class="phantom-line-money">
          <span class="phantom-pill"><span class="p-l">Cash</span><span class="p-v money-cash"><span>$</span>{{ fmtNum(cash) }}</span></span>
          <span class="phantom-pill"><span class="p-l">Bank</span><span class="p-v money-bank"><span>$</span>{{ fmtNum(bank) }}</span></span>
        </div>
        <div class="phantom-line3" v-if="showRank || showJob">
          <span v-if="showRank" class="phantom-pill"><span class="p-l">Rank</span><span class="p-v">{{ rank }}</span></span>
          <span v-if="showJob" class="phantom-pill"><span class="p-l">Job</span><span class="p-v">{{ job }}</span></span>
        </div>
        <div class="phantom-voice" :class="{ 'is-talking': talking }">
          <span class="phantom-mic">●</span><span>{{ voiceLabel }}</span>
        </div>
      </div>
    </div>
  `,
});
phantomTop.use(Quasar);
phantomTop.mount('#phantom-top-info');
