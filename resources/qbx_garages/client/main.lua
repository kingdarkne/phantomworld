...
--- External (e.g. dr-mainmenu): nearest non-depot garage menu — replaces qb-garages:client:openNearestPublicGarage
RegisterNetEvent('qbx_garages:client:openNearestGarage', function()
    if cache.vehicle then
        exports.qbx_core:Notify(locale('error.in_vehicle'), 'error')
        return
    end
    local pos = GetEntityCoords(PlayerPedId())
    local garages = lib.callback.await('qbx_garages:server:getGarages', false)
    local bestName, bestGarage, bestIdx, bestDist
    for name, garage in pairs(garages) do
        if garage.type ~= GarageType.DEPOT and garage.accessPoints then
            for i = 1, #garage.accessPoints do
                local ac = garage.accessPoints[i].coords
                local gp = vec3(ac.x, ac.y, ac.z)
                local d = #(pos - gp)
    ...
    if not bestName or not bestGarage then
        exports.qbx_core:Notify('No garage found.', 'error')
        return
    end
    if bestDist > 75.0 then
        exports.qbx_core:Notify('Move closer to a garage.', 'error')
        return
    end
    if not checkCanAccess(bestGarage) then return end
    openGarageMenu(bestName, bestGarage, bestIdx)
end)