...
-- Spawn UI is owned by qbx_spawn (map + last location + properties). This event remains for older scripts / qbx_core fallbacks.
RegisterNetEvent('apartments:client:setupSpawnUI', function(cData)
    if GetResourceState('qbx_spawn') == 'started' then
        TriggerEvent('qb-spawn:client:setupSpawns')
        return
    end
    local cid = type(cData) == 'table' and cData.citizenid or cData
    local result = lib.callback.await('apartments:GetOwnedApartment', false, cid)
    if result then
