...
function GetPlayerVehicleFilter(source, garageName)
    ...
    -- No loaded session (disconnect, race): avoid indexing nil player; filter matches nothing.
    if not player then
        return {
            citizenid = '__qbx_garages_none__',
            states = garage.states or VehicleState.GARAGED,
            garage = not garage.skipGarageCheck and garageName or nil,
        }
    end
    local filter = {}
