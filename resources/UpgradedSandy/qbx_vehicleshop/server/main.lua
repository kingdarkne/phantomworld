...
-- One-time DB: `vehshop.sql` also defines this; creating here fixes fresh DBs (starter pack / financing checks).
MySQL.ready.await()
do
    local ddl = [[CREATE TABLE IF NOT EXISTS `vehicle_financing` (
    ...
    CONSTRAINT `fk_vehicle_financing_vehicle` FOREIGN KEY (`vehicleId`) REFERENCES `player_vehicles` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;]]
    local ok, err = pcall(function()
        MySQL.query.await(ddl)
    end)
    if not ok then
        lib.print.error(('[qbx_vehicleshop] Could not ensure vehicle_financing table (run resources/qbx_vehicleshop/vehshop.sql manually): %s'):format(tostring(err)))
    end
end
