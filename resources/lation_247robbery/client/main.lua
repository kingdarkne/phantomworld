...
-- Spawn store clerk ped at register location
local function spawnStorePed(coord)
    local pedModel = `mp_m_shopkeep_01`
    lib.requestModel(pedModel, 5000)
    local ped = CreatePed(4, pedModel, coord.x, coord.y, coord.z - 1.0, 0.0, false, false)
    SetEntityHeading(ped, math.random(0, 360))
    SetEntityInvincible(ped, true)
    FreezeEntityPosition(ped, true)
    SetBlockingOfNonTemporaryEvents(ped, true)
    TaskPlayAnim(ped, "missheistdockssetup1clipboard@base", "base", 8.0, 0.0, -1, 1, 0, false, false, false)
