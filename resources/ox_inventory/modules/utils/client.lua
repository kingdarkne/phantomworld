...
-- Weapon wheel inventory sync (ported & adapted from Fourfinger92/ox_inventory-meets-weaponwheel).
-- Keeps GTA weapon wheel populated from ox_inventory weapons, while still using ox_inventory for durability/ammo/components.
Utils.weaponSyncDisabled = false
Utils.weaponWheelHotbarMode = false
...
local function syncWeaponWheelWeapons()
    local oldCache = weaponWheelCache
    local newCache = {}

    -- If disabled, keep a clean state (only ox_inventory equipped weapon should exist).
    if Utils.weaponSyncDisabled then
        weaponWheelCache = {}
        -- Remove any previously seeded weapons.
        for hash in pairs(oldCache) do
            if HasPedGotWeapon(cache.ped, hash, false) then
    ...
    -- Ox inventory owns the weapon state; we only "seed" the GTA wheel with the weapons in inventory.
    ...
    for _, v in pairs(items) do
        if type(v) == 'table' and type(v.name) == 'string' and v.slot and v.name:sub(1, 7) == 'WEAPON_' and v.name ~= 'WEAPON_PETROLCAN' then
            ...
            if not HasPedGotWeapon(cache.ped, hash, false) then
                GiveWeaponToPed(cache.ped, hash, 0, false, false)
            end
        end
    end

    -- Remove weapons that were previously seeded but are no longer in inventory.
    for hash in pairs(oldCache) do
        if not newCache[hash] and HasPedGotWeapon(cache.ped, hash, false) then
            RemoveWeaponFromPed(cache.ped, hash)
        end
    end

    weaponWheelCache = newCache
end
...
exports('Weaponsyncdisable', function(state)
    if state == nil then state = Utils.weaponSyncDisabled end
    Utils.weaponSyncDisabled = state
    if EnableWeaponWheel and not weaponWheelOverride then
        syncWeaponWheelWeapons()
    end
end)

exports('WeaponWheelandHotbar', function(state)
    if state == nil then state = Utils.weaponWheelHotbarMode end
    Utils.weaponWheelHotbarMode = state
    Utils.WeaponWheel(state, true)
end)
