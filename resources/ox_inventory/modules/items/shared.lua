...
-- QB-compatible items (github qbcore-framework/qb-core shared/items.lua → data/items_from_qb.lua via tools/qb_to_ox_items.mjs)
for k, v in pairs(lib.load('data.items_from_qb') or {}) do
	if not ItemList[k] then
		v.name = k
		local success, response = pcall(newItem, v)
		if not success then
			warn(('An error occurred while creating item "%s" (items_from_qb)!\n^1SCRIPT ERROR: %s^0'):format(k, response))
		end
	end
end
