		...
		for _, v in pairs(data) do
			if type(v) == 'number' or not v.count or not v.slot then
				if server.convertInventory then
					...
					warn(('Inventory for player.%s (%s) contains invalid data. Ensure you have converted inventories to the correct format. Starting with empty inventory.'):format(player.source, GetPlayerName(player.source)))
					break
				end
			else
