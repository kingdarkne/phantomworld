...
---@param slot number
---@return boolean?
local function useSlot(slot, noAnim, fromWeaponWheel)
	...
	if canUseItem(data.ammo and true) then
		...
		elseif data.weapon then
			if (EnableWeaponWheel and not fromWeaponWheel) or not plyState.canUseWeapons then return end
...
-- Weapon wheel integration: when GTA wheel selection changes, equip the matching ox_inventory weapon slot.
local lastWheelHash = `WEAPON_UNARMED`

CreateThread(function()
	while true do
		Wait(50)

		if not EnableWeaponWheel or invOpen or invBusy or IsNuiFocused() or lib.progressActive() then
			lastWheelHash = `WEAPON_UNARMED`
		else
			local selected = GetSelectedPedWeapon(playerPed)

			if selected ~= lastWheelHash then
				lastWheelHash = selected

				-- Unarmed -> disarm tracked weapon.
				if selected == `WEAPON_UNARMED` then
					if currentWeapon then
						currentWeapon = Weapon.Disarm(currentWeapon, true)
					end
				else
					local cacheTable = Utils.GetWeaponWheelCache and Utils.GetWeaponWheelCache() or {}
					local entry = cacheTable and cacheTable[selected]

					if entry and entry.slot then
						-- Equip via the normal useSlot path, but allow it while weapon wheel is enabled.
						CreateThread(function()
							useSlot(entry.slot, true, true)
						end)
					end
				end
			end
