...
function spawnPlayer(spawnIdx, cb)
    ...
    Citizen.CreateThread(function()
        local ok, err = pcall(function()
            ...
            if not spawnIdx then
                if #spawnPoints == 0 then
                    Citizen.Trace("spawnmanager: no spawn points registered; cannot spawn\n")
                    return
                end
            ...
            -- validate before reading spawn.skipFade (nil spawn used to error here and leave spawnLock stuck)
            if not spawn then
        ...
        if not ok then
            Citizen.Trace("spawnmanager spawn error: " .. tostring(err) .. "\n")
        end
