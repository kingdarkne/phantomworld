...
function InitAppearance()
    ...
    lib.callback("illenium-appearance:server:getAppearance", false, function(appearance)
        ...
        -- Lets other resources (e.g. starter car) open after skin is applied for returning characters
        TriggerEvent("illenium-appearance:client:appearanceLoaded")
    end)
    ResetBlips()
    if Config.BossManagedOutfits then
        Management.AddItems()
...
function InitializeCharacter(gender, onSubmit, onCancel)
    ...
    client.startPlayerCustomization(function(appearance)
        if (appearance) then
            ...
            TriggerEvent("illenium-appearance:client:characterCreated")
            if onSubmit then
                onSubmit()
            end
