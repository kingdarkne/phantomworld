...
function makeSafeTranslations(t) {
    const base = Object.assign({}, TRANSLATION_FALLBACKS, t || {});
    // If any key is missing, prevent JS string concatenation from producing literal "undefined".
...
translations = makeSafeTranslations({});

$(document).ready(function (){
    window.addEventListener('message', function (event) {
        var data = event.data;
        if (data.action == "ui") {
			NChar = Number(data.nChar);
            if (!Number.isFinite(NChar) || NChar < 1) NChar = 2;
            EnableDeleteButton = data.enableDeleteButton === true;

            translations = makeSafeTranslations(data.translations);
            if (data.toggle) {
                $('.container').show();
                ...
                setTimeout(function(){
                    ...
                    $.post('https://dr-multicharacter/setupCharacters');
                    setTimeout(function(){
                        ...
                        $.post('https://dr-multicharacter/removeBlur');
                        SetLocal()
                    }, 500);
                }, 2000);
            } else {
...
$('.disconnect-btn').click(function(e){
    ...
    $.post('https://dr-multicharacter/closeUI');
    $.post('https://dr-multicharacter/disconnectButton');
});
  ...
  function SetLocal() {
   ...
   function setupCharacters(characters,photo1,photo2,photo3,photo4) {
    ...
    $.each(characters, function(index, char){
        var slotId = index + 1;
        $('#char-'+slotId).html("");
        $('#char-'+slotId).data("citizenid", char && char.citizenid ? char.citizenid : "");
        var firstName = (char && char.charinfo && char.charinfo.firstname) ? char.charinfo.firstname : "";
        var lastName = (char && char.charinfo && char.charinfo.lastname) ? char.charinfo.lastname : "";
        var citizenId = (char && char.citizenid) ? char.citizenid : "";
        if(slotId == 1){
        ...
        if(tempUrl == translations["default_image"]){
            ...
            $('#char-'+slotId).html('<div class="character-div"><div class="user"> <img src="'+tempUrl +'" alt="'+slotId +'photo" /></div><span id="slot-name">'+firstName+' '+lastName+'<span id="cid">' + citizenId + '</span></span><div class="user3"><img  src="' + translations["default_right_image"] + '" alt="plus" /></div></div>  <div class="btns" style=""> <div class="character-btn" id="select" style="display: block;"><p id="select-text">'+translations["select"]+'</p></div> </div>');
            $('#char-'+slotId).data('cData', char)
            $('#char-'+slotId).data('cid', citizenId)
        }else{
...
$(document).on('click', '.character', function(e) {
    ...
    if (selectedChar === null) {
        ...
        if ((selectedChar).data('cid') == "") {
            ...
            $.post('https://dr-multicharacter/cDataPed', JSON.stringify({
                cData: cDataPed
            }));
        } else {
            $(selectedChar).addClass("char-selected");
        ...
        $(selectedChar).find(".btns").append('<div class="character-btn" id="select" style="display: block;"><p id="select-text">'+translations["select"]+'</p></div>');
...
$(document).on('click', '#create', function (e) {
    ...
    if (!firstname || !lastname || !nationality || !birthdate){
        ...
        $('.error-title').html('<span class="material-symbols-outlined">'+translations["error_title"])
    ...
    $.post('https://dr-multicharacter/createNewCharacter', JSON.stringify({
        firstname: firstname,
        lastname: lastname,
...
$(document).on('click', '#accept-delete', function(e){
    $.post('https://dr-multicharacter/removeCharacter', JSON.stringify({
...
$(document).on('click', '#play', function(e) {
    e.preventDefault();
    e.stopPropagation();
    var charData = selectedChar ? $(selectedChar).data('cid') : '';
    var cData = selectedChar ? $(selectedChar).data('cData') : null;

    if (selectedChar !== null) {
        if (charData !== "" && charData !== undefined && cData) {
            $.post('https://dr-multicharacter/selectCharacter', JSON.stringify({
                cData: cData
            }));
            setTimeout(function(){
 ...
 $(".char-register-inputsss").html('<div class="input-div2"><i class="fa-solid fa-calendar calendar-icon"></i><input type="date" id="birthdate" style="color: rgb(255, 255, 255);" placeholder="'+translations["create_birthday"]+'" value="2005-05-05" min="1900-01-01" max="2023-12-31" class="char-reg-input"></div>');
    ...
    $("#close-reg").html('<p>'+translations["cancel"]+'</p>');
            qbMultiCharacters.fadeInDown('.character-register', '50%', 1);
...
qbMultiCharacters.fadeOutDown = function(element, percent, time) {
    $(element).stop(true, true).css({"display":"block"}).animate({opacity: 0}, time, function(){
        $(element).css({"display":"none", "opacity":"1"});
    });
}

qbMultiCharacters.fadeInDown = function(element, percent, time) {
    var displayVal = $(element).hasClass('character-register') ? 'flex' : 'block';
    $(element).stop(true, true).css({"display": displayVal, "opacity":"0"}).animate({opacity: 1}, time);
}
...
qbMultiCharacters.resetAll = function() {
    $('.characters-list').hide();
    $('.characters-list').css({"top":"50%", "bottom":"auto", "left":"auto", "right":"3%", "opacity":"1"});
    $('.bar').hide();
    $('.bar').css({"top":"auto", "bottom":"31%", "left":"50%", "opacity":"1"});
    $('.bar2').hide();
    $('.bar2').css({"top":"auto", "bottom":"31%", "left":"50%", "opacity":"1"});
    $('.characters-text').hide();
    $('.characters-text').css({"top":"auto", "bottom":"28%", "left":"50%", "opacity":"1"});
    $('.characters-text2').hide();
    $('.characters-text2').css({"top":"auto", "bottom":"25.5%", "left":"50%", "opacity":"1"});
    $('.characters-icon').hide();
    $('.characters-icon').css({"top":"auto", "bottom":"28%", "left":"50%", "opacity":"1"});
    $('.character-info').hide();
    $('.character-info').css({"top":"10%", "opacity":"1"});
    $('.character-register').hide().css({"top":"50%", "left":"50%", "opacity":"1"});
