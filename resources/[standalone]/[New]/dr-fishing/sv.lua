...
QBCore.Functions.CreateUseableItem = QBCore.Functions.CreateUseableItem or function(name, cb)
...
QBCore.Functions.CreateUseableItem("fishingrod", function(source, item)
	...
	if Player.Functions.GetItemBySlot(item.slot) ~= nil then
        TriggerClientEvent('dr-fishing:start', source)
    end
end)

RegisterNetEvent('dr-fishing:removebait', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local quantity = 1

    exports.ox_inventory:RemoveItem(source, 'fishbait', quantity)
end)

RegisterNetEvent('dr-fishing:reward', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('fishbasket:craft', function(fishName, requiredAmount)
    ...
    if Player then
        ...
        if hasFish then
            exports.ox_inventory:RemoveItem(source, fishName, requiredAmount)
...
RegisterNetEvent('jomidar:delivery', function()
    ...
    if Player then
        ...
        if hasFish then
            exports.ox_inventory:RemoveItem(source, 'fishcontainer', 1)
