...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    if GetResourceState('illenium-appearance') == 'started' then
        ...
        if player.metadata.tracker then
            pcall(function()
                exports['illenium-appearance']:setPedComponent(ped, { component_id = 7, drawable = 13, texture = 0 })
            end)
        else
            pcall(function()
                exports['illenium-appearance']:setPedComponent(ped, { component_id = 7, drawable = 0, texture = 0 })
            end)
        end
    else
        if player.metadata.tracker then
            TriggerEvent('qb-clothing:client:loadOutfit', {
                outfitData = { ["accessory"] = { item = 13, texture = 0 } }
            })
        else
            TriggerEvent('qb-clothing:client:loadOutfit', {
                outfitData = { ["accessory"] = { item = -1, texture = 0 } }
            })
        end
    end
