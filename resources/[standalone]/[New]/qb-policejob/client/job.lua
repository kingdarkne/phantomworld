...
if Config.UseTarget then
    ...
    stashCombo:onPlayerInOut(function(isPointInside, _, _)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                QBCore.Functions.DrawText(Lang:t('info.stash_enter'), 'left')
                stash()
            end
        else
            inStash = false
    ...
    trashCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inTrash = true
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                QBCore.Functions.DrawText(Lang:t('info.trash_enter'),'left')
                trash()
            end
        else
            inTrash = false
    ...
    fingerprintCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inFingerprint = true
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                QBCore.Functions.DrawText(Lang:t('info.scan_fingerprint'),'left')
                fingerprint()
            end
        else
            inFingerprint = false
    ...
    armouryCombo:onPlayerInOut(function(isPointInside)
        if isPointInside then
            inArmoury = true
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                QBCore.Functions.DrawText(Lang:t('info.enter_armory'),'left')
                armoury()
            end
        else
            inArmoury = false
    ...
    garageCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.name == 'police' and PlayerJob.onduty then
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    QBCore.Functions.DrawText(Lang:t('info.store_veh'), 'left')
            garage()
                else
        ...
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty then
                if IsPedInAnyHeli(PlayerPedId()) then
                    ...
                    QBCore.Functions.DrawText(Lang:t('info.store_heli'), 'left')
                    heli()
                else
                    QBCore.Functions.DrawText(Lang:t('info.take_heli'), 'left')
                    heli(zone.data.zone)
                end
            end
        else
    ...
    impoundCombo:onPlayerInOut(function(isPointInside, point)
        if isPointInside then
            ...
            if PlayerJob.type == "leo" and PlayerJob.onduty and IsPedInAnyVehicle(PlayerPedId(), false) then
                QBCore.Functions.DrawText(Lang:t('info.impound_veh'), 'left')
                impound()
            else
                local currentSelection = 0
