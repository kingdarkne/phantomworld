...
MC.AdminMenu.BuildPlayerList = function () {
  ...
  for (let i = 0; i < MC.AdminMenu.Players.length; i++) {
    ...
    let PlayerItem = `<div class="admin-menu-player waves-effect waves-light" id="admin-player-${
      Player["ServerId"]
    }">
                            <div class="admin-menu-player-pintarget ${
                              MC.AdminMenu.PinnedTargets[Player["License"]]
                                ? "pinned"
                                : ""
                            }">${
      MC.AdminMenu.PinnedTargets[Player["License"]]
        ...
        : `<i class="fa-regular fa-map-pin"></i>`
                            ...
                            <div class="admin-menu-player-id">(${
                              Player["ServerId"]
                            })</div>
                            <div class="admin-menu-player-name">${
                              Player["Name"]
                            }</div>
                            <div class="admin-menu-player-steam">[${
                              Player["Steam"]
                            }]</div>
                        </div>`;
...
MC.AdminMenu.BuildPinnedPlayerList = function () {
  ...
  $.each(MC.AdminMenu.PinnedTargets, function (Key, Value) {
    if (Value) {
      $.post(
        `https://${GetParentResourceName()}/GetCharData`,
        JSON.stringify({
          Identifier: Key,
        }),
        function (pData) {
          if (pData != undefined) {
            ...
            $(".menu-pinned-players-list")
              .append(`<div class="menu-pinned-player" data-PinnedPlayer="${pData.Name}">
...
$(document).on("click", ".admin-menu-player-pintarget", function (e) {
  ...
  if ($(this).hasClass("pinned")) {
    ...
    $.post(
      `https://${GetParentResourceName()}/ToggleKVP`,
      JSON.stringify({
        Type: "pinned_targets",
        Id: Data.License,
        Toggle: false,
    ...
    $.post(
      `https://${GetParentResourceName()}/ToggleKVP`,
      JSON.stringify({ Type: "pinned_targets", Id: Data.License, Toggle: true })
    );
