...
RegisterCommand('adminmenu', function(source, args, RawCommand) TriggerServerEvent('j0-admin/server/try-open-menu', true) end, false)

RegisterNetEvent('j0-admin/client/try-open-menu', function(KeyPress)
    -- Refresh ACE permissions so client matches server (fixes menu not opening after load)
    QBCore.Functions.TriggerCallback('j0-adminmenu/server/get-permission', function(pGroup)
        Group = pGroup
