...
-- QBCore compatibility is provided by shared/qbx_compat.lua (QBX-native).
QBShared = QBShared or (QBCore and QBCore.Shared) or {}
...
function IsPlayerAdmin()
    local Rank, Bool = GetPlayerRank()
    if Rank == nil then
        return false
    end
    for i = 1, #Config.Settings['AdminGroups'] do
        local ag = Config.Settings['AdminGroups'][i]
        if Bool ~= nil and Rank == ag and Bool then
            return true
        end
        if Bool == nil and Rank == ag then
            return true
        end
    end
    return false
...
function GetCommands()
    ...
    if type(Config.CommandList) == 'table' and next(Config.CommandList) ~= nil then
...
function GetPlayerRank()
    local Prom = promise:new()
    if Group == nil then
        -- Try to get group from qbx_core if not already set
        local success, result = pcall(function()
            return exports.qbx_core:GetPlayerData()
        end)
        if success and result and result.group then
            Group = result.group
        else
            Prom:resolve(nil, nil)
            return Citizen.Await(Prom)
        end
    end
    if type(Group) ~= 'table' then -- Old QB: single string
        Prom:resolve(Group, nil)
        return Citizen.Await(Prom)
    end
    -- New QB: return first admin rank that matches AdminGroups
    for Rank, Bool in pairs(Group) do
        if Bool then
            for i = 1, #Config.Settings['AdminGroups'] do
                if Rank == Config.Settings['AdminGroups'][i] then
...
function GetMoneyTypes()
    ...
    local Types = nil
    if QBCore and QBCore.Config then
        -- qbx_core (older/common): QBCore.Config.Money.MoneyTypes
        Types = (QBCore.Config.Money and QBCore.Config.Money.MoneyTypes) or Types
        -- qbx_core style config tables sometimes end up exposed in lowercase
        Types = (QBCore.Config.money and QBCore.Config.money.moneyTypes) or Types
        -- some forks expose moneyTypes directly
        Types = QBCore.Config.MoneyTypes or Types
        Types = QBCore.Config.moneyTypes or Types
    end

    -- Final fallback so the menu doesn't crash on non-qbox_core frameworks
    if Types == nil then
        Types = { cash = true, bank = true, crypto = true }
    end

    if type(Types) == 'table' then
        -- handle both { cash = 500, bank = 5000 } and { 'cash', 'bank', 'crypto' }
        local isArray = (#Types > 0)
        if isArray then
            for _, v in ipairs(Types) do
                MoneyTypes[#MoneyTypes + 1] = { Text = tostring(v) }
            end
        else
            for k, _ in pairs(Types) do
                MoneyTypes[#MoneyTypes + 1] = { Text = tostring(k) }
            end
        end
        ...
        DebugPrint('money', 'Could not determine money types (expected table). Falling back to defaults.')
        Prom:resolve({ { Text = 'cash' }, { Text = 'bank' }, { Text = 'crypto' } })
    end
    return Citizen.Await(Prom)
end
