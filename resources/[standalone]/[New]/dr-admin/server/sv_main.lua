...
RegisterNetEvent('jomidar:additem', function(ItemName, ItemAmount)
    ...
    if GetResourceState('ox_inventory') == 'started' then
        ...
        TriggerClientEvent('ox_lib:notify', src, { description = ('Received %dx %s'):format(ItemAmount, ItemName), type = 'success' })
    else
        local Player = QBCore.Functions.GetPlayer(src)
        if not Player then return end
        Player.Functions.AddItem(ItemName, ItemAmount)
        TriggerClientEvent('QBCore:Notify', src, ('Received %dx %s'):format(ItemAmount, ItemName), 'success')
    end
end)
...
RegisterNetEvent("j0-admin/server/set-environment", function(Weather, Hour, Minute)
    ...
    if Weather ~= nil then
        local UpdatedWeather = exports['dr-weathersync']:setWeather(Weather)
    ...
    if Hour ~= nil and Minute ~= nil then
        local UpdatedTime = exports['dr-weathersync']:setTime(Hour, Minute)
...
RegisterNetEvent("j0-admin/server/open-clothing", function(ServerId)
    ...
    if GetResourceState('illenium-appearance') == 'started' then
        TriggerClientEvent('illenium-appearance:client:openClothingShop', Source, false)
    else
