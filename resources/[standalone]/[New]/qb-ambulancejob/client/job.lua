    ...
    CreateThread(function()
        ...
        QBCore.Functions.GetPlayerData(function(PlayerData)
            ...
            if (not PlayerData.metadata["inlaststand"] and PlayerData.metadata["isdead"]) then
                -- If player logs in while flagged as dead, send them to hospital instead of reapplying death state
            ...
            if result then
                for k, v in pairs(result) do
                    ...
                    elseif result["WEAPONWOUNDS"] then
                        for _, v2 in pairs(result["WEAPONWOUNDS"]) do
                            ...
                            TriggerEvent('chat:addMessage', {
                                ...
                                args = { Lang:t('info.status'), QBCore.Shared.Weapons[v2].damagereason, '%H:%M' }
                            })
                        end
                    elseif result["BLEED"] > 0 then
                        ...
                        TriggerEvent('chat:addMessage', {
                            template = '<div class="chat-message system"><i class="fas fa-cog"></i> <b><span style="color: #df7b00">SYSTEM</span>&nbsp;<span style="font-size: 14px; color: #e1e1e1;">{0}</span></b><div style="margin-top: 5px; font-weight: 300;">{1}</div></div>',
                            args = { Lang:t('info.status'), Lang:t('info.is_status', { status = Config.BleedingStates[v].label }), '%H:%M' }
                        })
                    else
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                QBCore.Functions.KeyPressed(38)
...
CreateThread(function()
    for k, v in pairs(Config.Locations["vehicle"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                QBCore.Functions.DrawText(Lang:t('text.veh_button'), 'left')
                EMSVehicle(k)
            else
                CheckVehicle = false
    ...
    for k, v in pairs(Config.Locations["helicopter"]) do
        ...
        boxZone:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" and onDuty then
                QBCore.Functions.DrawText(Lang:t('text.heli_button'), 'left')
                EMSHelicopter(k)
            else
                CheckHeli = false
                QBCore.Functions.HideText()
    ...
    CreateThread(function()
        ...
        signCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if not onDuty then
                    QBCore.Functions.DrawText(Lang:t('text.onduty_button'), 'left')
                    EMSControls("sign")
                else
                    QBCore.Functions.DrawText(Lang:t('text.offduty_button'), 'left')
                    EMSControls("sign")
                end
            else
                check = false
        ...
        stashCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    QBCore.Functions.DrawText(Lang:t('text.pstash_button'), 'left')
                    EMSControls("stash")
                end
            else
                check = false
        ...
        armoryCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    QBCore.Functions.DrawText(Lang:t('text.armory_button'), 'left')
                    EMSControls("armory")
                end
            else
                check = false
        ...
        roofCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    QBCore.Functions.DrawText(Lang:t('text.elevator_main'), 'left')
                    EMSControls("main")
                else
                    QBCore.Functions.DrawText(Lang:t('error.not_ems'), 'left')
                end
            else
                check = false
                QBCore.Functions.HideText()
        ...
        mainCombo:onPlayerInOut(function(isPointInside)
            if isPointInside and PlayerJob.name == "ambulance" then
                if onDuty then
                    QBCore.Functions.DrawText(Lang:t('text.elevator_roof'), 'left')
                    EMSControls("roof")
                else
                    QBCore.Functions.DrawText(Lang:t('error.not_ems'), 'left')
                end
