QBCore = QBCore or { Functions = {}, Shared = { Items = {}, Weapons = {} } }
...
RegisterNetEvent('hospital:client:RespawnAtHospital', function()
    -- Clear deathscreen and revive player, then let server handle billing/teleport
    TriggerEvent('qb-deathscreen:revive')
    TriggerEvent('hospital:client:Revive')
...
RegisterNetEvent('hospital:client:TeleportOutside', function(coords)
    local ped = PlayerPedId()
    local x, y, z, w = coords.x, coords.y, coords.z, coords.w or coords.h or GetEntityHeading(ped)
    ...
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            ...
            checkingCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    if doctorCount >= Config.MinimalDoctors then
                        QBCore.Functions.DrawText(Lang:t('text.call_doc'),'left')
                        CheckInControls("checkin")
                    else
                        QBCore.Functions.DrawText(Lang:t('text.check_in'), 'left')
                        CheckInControls("checkin")
                    end
                else
                    listen = false
        ...
        for k, v in pairs(Config.Locations["beds"]) do
            ...
            bedCombo:onPlayerInOut(function(isPointInside)
                if isPointInside then
                    QBCore.Functions.DrawText(Lang:t('text.lie_bed'), 'left')
