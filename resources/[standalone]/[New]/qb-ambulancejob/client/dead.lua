...
function DeathTimer()
    ...
    while isDead do
        ...
        if deathTime > 0 then
            ...
            -- keep deathscreen UI timer in sync
            TriggerEvent('qb-deathscreen:updateRes', deathTime)
        else
            if not isInHospitalBed then
                TriggerEvent("hospital:client:RespawnAtHospital")
            end
            -- once we trigger a respawn, stop the loop
            break
        end
    end
end
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Config.MinimumRevive then
                  --  DrawTxt(0.94, 1.44, 1.0, 1.0, 0.6, Lang:t('info.bleed_out', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                    TriggerEvent('qb-deathscreen:die',LaststandTime)
                else
