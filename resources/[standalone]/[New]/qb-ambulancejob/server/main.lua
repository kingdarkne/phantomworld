...
QBCore.Functions.CreateUseableItem = QBCore.Functions.CreateUseableItem or function(name, cb) exports.qbx_core:CreateUseableItem(name, cb) end
QBCore.Functions.CreateCallback = QBCore.Functions.CreateCallback or function(name, cb) if lib and lib.callback then lib.callback.register(name, cb) end end
QBCore.Functions.HasItem = QBCore.Functions.HasItem or function(src, item, amount)
...
QBCore.Functions.GetIdentifier = QBCore.Functions.GetIdentifier or function(src, idType)
    for _, identifier in ipairs(GetPlayerIdentifiers(src)) do
        if identifier:find((idType or '') .. ':', 1, true) == 1 then return identifier end
    end
end
QBCore.Functions.Notify = QBCore.Functions.Notify or function(src, msg, nType)
...
QBCore.Commands.Add = QBCore.Commands.Add or function(name, _, _, _, cb, permission)
    RegisterCommand(name, function(source, args)
        if source == 0 or IsPlayerAceAllowed(source, permission or 'admin') or IsPlayerAceAllowed(source, 'admin') then cb(source, args) end
    end, false)
end
local doctorCount = 0
...
RegisterNetEvent('hospital:server:RespawnAtHospital', function()
	...
	if Player.PlayerData.metadata["injail"] > 0 then
		...
		-- Not in jail: respawn the player just outside the main hospital instead of in a bed
		local outside = Config.Locations["stations"][1] and Config.Locations["stations"][1].coords
		if outside then
			TriggerClientEvent('hospital:client:TeleportOutside', src, outside)
		end
