...
QBCore.Functions.GetQBPlayers = QBCore.Functions.GetQBPlayers or function() return exports.qbx_core:GetQBPlayers() end
QBCore.Functions.CreateCallback = QBCore.Functions.CreateCallback or function(name, cb)
    if lib and lib.callback then lib.callback.register(name, cb) end
end
QBCore.Functions.GetIdentifier = QBCore.Functions.GetIdentifier or function(src, idType)
    for _, identifier in ipairs(GetPlayerIdentifiers(src)) do
        if identifier:find((idType or '') .. ':', 1, true) == 1 then
            return identifier
        end
    end
    return nil
...
QBCore.Commands.Add = QBCore.Commands.Add or function(name, _, _, _, cb, permission)
    RegisterCommand(name, function(source, args)
        if source == 0 or IsPlayerAceAllowed(source, permission or 'admin') or IsPlayerAceAllowed(source, 'admin') then
            cb(source, args)
        end
    end, false)
end
...
local MugShotWebhook = 'https://discord.com/api/webhooks/1472834043820380396/gdwlA1vDN3id4leLzFUHkh-oDX2VrzfzwySTzd9G4VYx2u-9eJGaOsWwDRe-MZmiqj5g'

-- Clock-in notifications for duty. Add a Discord webhook.
-- Command /mdtleaderboard, will display top players per clock-in hours.
local ClockinWebhook = 'https://discord.com/api/webhooks/1472833549936754728/VzRHFBbY4KytG3N5zsclUC5Lv-ztyudvb_2jze77fGw7oCi6dJqWQkOKNNCJg8daq1mV'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1477376673115537508/wC7YU0ZcXTRIkkS1mweTGpRs-JY9STEGHR6BTs9yoH8uJA3TbbyPjWMTaKxzveLwRyN7'
--------------------------------
...
QBCore.Functions.CreateCallback('mdt:server:GetProfileData', function(source, cb, sentId)
	...
	-- Attach live wanted data from dr-wanted if resource is running and player is online
	if GetResourceState('dr-wanted') == 'started' then
		local onlineTarget = QBCore.Functions.GetPlayerByCitizenId(target.citizenid)
		if onlineTarget then
			local srcId = onlineTarget.PlayerData.source
			local wanted = exports['dr-wanted']:GetWantedData(srcId)
			if wanted then
				person.wanted_level = wanted.level or 0
				person.wanted_points = wanted.points or 0
			end
		end
	end
