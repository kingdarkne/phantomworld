...
--- Label/icon for menus: ox_inventory when `Config.Inv == "ox"`, else QBCore.Shared.Items.
function GetJimItemLabel(item)
	if Config.Inv == "ox" then
		local d = exports.ox_inventory:Items(item)
		return (d and d.label) or tostring(item)
	end
	local s = Core.Shared.Items[item]
	return (s and s.label) or tostring(item)
end

function GetJimItemIcon(item)
	if Config.Inv == "ox" then
		local d = exports.ox_inventory:Items(item)
		if d and d.client and d.client.image then
			return ("nui://ox_inventory/web/images/%s"):format(d.client.image)
		end
		return ("nui://ox_inventory/web/images/%s.png"):format(item)
	end
	local s = Core.Shared.Items[item]
	if not s or not s.image then
		return ("nui://%s%s.png"):format(Config.img, tostring(item))
	end
	return ("nui://%s%s"):format(Config.img, s.image)
end