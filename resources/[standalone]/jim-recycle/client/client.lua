...
RegisterNetEvent('jim-recycle:Selling:Menu', function(data)
	...
	for item, price in pairsByKeys(Config.Prices) do
		sellMenu[#sellMenu+1] = {
			...
			icon = GetJimItemIcon(item),
			header = GetJimItemLabel(item),	txt = Loc[Config.Lan].menu["sell_all"]..price..Loc[Config.Lan].menu["each"],
			params = { event = "jim-recycle:SellAnim", args = { Ped = data.Ped, item = item } },
			title = GetJimItemLabel(item), description = Loc[Config.Lan].menu["sell_all"]..price..Loc[Config.Lan].menu["each"],
...
RegisterNetEvent('jim-recycle:Trade:Menu', function(data)
	if Selling then return end
	local tradeMenu = {}
	local icon = GetJimItemIcon("recyclablematerial")
	if Config.Menu == "qb" then
...
RegisterNetEvent('jim-recycle:Bottle:Menu', function(data)
	...
	for _, item in pairsByKeys(Config.BottleBankTable) do
		tradeMenu[#tradeMenu+1] = {
			disabled = not HasItem(item, 1),
			icon = GetJimItemIcon(item),
			header = GetJimItemLabel(item), txt = Loc[Config.Lan].menu["sell_all"]..Config.Prices[item]..Loc[Config.Lan].menu["each"],
			params = { event = "jim-recycle:SellAnim", args = { item = item, Ped = data.Ped } },
			title = GetJimItemLabel(item), description = Loc[Config.Lan].menu["sell_all"]..Config.Prices[item]..Loc[Config.Lan].menu["each"],
