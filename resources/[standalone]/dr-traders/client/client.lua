...
local function canInteract(trader, tradeName)
    local trade = exports['dr-trade']:getTrade(tradeName)
    if trade then
...
local function getOptions(trader)
    ...
    if trader.trades then
        for i,trade in pairs(trader.trades) do
            local option = {
                type = "client",
                event = "dr-trade:client:attemptTrade",
            ...
            if trade.allowBatchTrade then
                ...
                local batchOption = {
                    type = "client",
                    event = "dr-traders:client:openBatchTrade",
...
RegisterNetEvent('dr-traders:client:openBatchTrade', function(data)
    local trade =  exports['dr-trade']:getTrade(data.tradeName)
    local selectOptions = {}
    ...
    if trade.fromItems then
        ...
        if dialog ~= nil then
            ...
            TriggerEvent("dr-trade:client:attemptTrade", data, modifier)
        end
    else
        lib.notify({ title = 'Trade', description = "This trade doesn't have items", type = 'error' })
    end
 end)
