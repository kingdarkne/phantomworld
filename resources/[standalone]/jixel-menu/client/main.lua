...
local function exportHandler(exportName, func)
    AddEventHandler(('__cfx_export_%s_%s'):format("dr-menu", exportName), function(setCB)
        setCB(func)
    end)
end
...
if GetResourceState("qb-inventory") == "started" then
    inventoryLink = "nui://qb-inventory/html/images"
elseif GetResourceState("ox_inventory") == "started" then
    inventoryLink = "nui://ox_inventory/web/images"
elseif GetResourceState("ps-inventory") == "started" then
    inventoryLink = "nui://ps-inventory/html/images"
elseif GetResourceState("lj-inventory") == "started" then
    inventoryLink = "nui://lj-inventory/html/images"
end
...
local function openMenu(data, modifiers)
	...
	for _, v in pairs(data) do
		if v["icon"] then
			if QBCore.Shared.Items[tostring(v["icon"])] and not string.find(QBCore.Shared.Items[tostring(v["icon"])].image, "http") and not string.find(v["icon"], "//") then
                if Debug then
                    print("^5Debug^7: Converted icon url ^7'^6"..v["icon"].."^7' - '^6"..inventoryLink..QBCore.Shared.Items[tostring(v["icon"])].image.."^7'")
                end
                v["icon"] = inventoryLink..QBCore.Shared.Items[tostring(v["icon"])].image
			end
		end
...
RegisterNetEvent('dr-menu:client:openMenu', function(data) openMenu(data) end)

RegisterNetEvent('dr-menu:client:closeMenu', function() closeMenu() end)
...
exportHandler('closeMenu', function() closeMenu() end)
exportHandler('showHeader', function(data) showHeader(data) end)
