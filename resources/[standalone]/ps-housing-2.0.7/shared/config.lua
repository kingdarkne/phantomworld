...
-- QBX setup:
-- - Target/Notify/Radial: use ox_* stack (does not require qb-core bridge)
-- - Inventory: use ox_inventory (required by qbx_core)
...
-- qbx_properties / Qbox migration stores GTA Online interior ids as `properties.shell` (see QBOX README).
-- Those interiors use the same K4MB1 furnished mid apartment as "Apartment Furnished".
local _qbxAptShell = Config.Shells["Apartment Furnished"]
if _qbxAptShell then
    for _, interiorName in ipairs({
        ...
        Config.Shells[interiorName] = _qbxAptShell
    end
end

--- Stash options for a shell name (handles migrated qb-housing shells missing from Config.Shells).
function Config.GetShellStash(shell)
    local d = shell and Config.Shells[shell]
    if d and d.stash then return d.stash end
    local fb = Config.Shells['mlo'] or Config.Shells['Standard Motel']
    if fb and fb.stash then return fb.stash end
    return { maxweight = 100000, slots = 12 }
end
...
Config.FurnitureTypes = {
    ["storage"] = function(entity, property_id, shell, id)
        ...
        Framework[Config.Target].AddTargetEntity(entity, "Storage", "fas fa-box-open", function()
            local stashConfig = Config.GetShellStash(shell)
