...
QBCore = QBCore or { Functions = {}, Commands = {}, Shared = {} }
QBCore.Functions = QBCore.Functions or {}
QBCore.Functions.GetPlayer = QBCore.Functions.GetPlayer or function(source)
    return exports.qbx_core:GetPlayer(source)
end
QBCore.Functions.GetPlayerByCitizenId = QBCore.Functions.GetPlayerByCitizenId or function(citizenid)
    return exports.qbx_core:GetPlayerByCitizenId(citizenid)
end
QBCore.Functions.SetPlayerBucket = QBCore.Functions.SetPlayerBucket or function(source, bucket)
    ...
    MySQL.query('SELECT * FROM properties', {}, function(result)
        ...
        for _, v in pairs(result) do
            ...
            if v.shell == 'mlo' and owner then
                ...
                if DoorResource == 'qb' then
                    property:addMloDoorsAccess(owner)
                elseif DoorResource == 'ox' then
                    property:addMloDoorsAccess(owner)
                end
...
function RegisterProperty(propertyData, preventEnter, source)
    ...
    if source and propertyData.shell == 'mlo' then
        ...
        Wait(1000)
        -- Add owner to ox_doorlock doors (qb adds via saveNewDoor)
        if DoorResource == 'ox' and propertyData.owner then
            propertyData.property_id = tostring(id)
            local prop = Property:new(propertyData)
            prop:addMloDoorsAccess(propertyData.owner)
        end
    end
