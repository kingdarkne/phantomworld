...
function Property:new(propertyData)
    ...
    local stashConfig = Config.GetShellStash(propertyData.shell)
    if not Config.Shells[propertyData.shell] then
        print(('^3[ps-housing]^7 Unknown shell "%s" for property %s — using default stash. Add the shell to Config.Shells or fix `properties.shell` in the database.'):format(tostring(propertyData.shell), tostring(propertyData.property_id)))
    end

    local furnitures = propertyData.furnitures
    if type(furnitures) ~= 'table' then furnitures = {} end
...
function Property:PlayerEnter(src)
    ...
    if not isMlo then
        TriggerClientEvent('dr-weathersync:client:DisableSync', src)
    end
...
function Property:PlayerLeave(src)
    ...
    TriggerClientEvent('dr-weathersync:client:EnableSync', src)
...
RegisterNetEvent("ps-housing:server:openQBInv", function(data)
    local src = source
    local stashId, stashData, propertyId = data.stashId, data.stashData, data.propertyId
    ...
    if GetResourceState('ox_inventory') == 'started' and exports.ox_inventory and exports.ox_inventory.openInventory then
        exports.ox_inventory:openInventory('stash', stashId)
        return
    end

    if GetResourceState('qb-inventory') == 'started' and exports['qb-inventory'] and exports['qb-inventory'].OpenInventory then
        exports['qb-inventory']:OpenInventory(src, stashId, stashData)
    else
        Framework[Config.Notify].Notify(src, "Inventory is not available.", "error")
    end
end)
