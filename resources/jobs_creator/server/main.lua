-- QBX-native: remove qb-core dependency (qbx_core + ox_lib callbacks)
...
lib.callback.register('jobcreator:getRanksForJob', function(_, jobName)
    return MySQL.query.await('SELECT * FROM basjob_ranks WHERE job_name = ?', { jobName }) or {}
end)

lib.callback.register('jobcreator:getJobs', function()
    local results = MySQL.query.await('SELECT job_name, job_label FROM basjob_ranks GROUP BY job_name, job_label') or {}
    local jobs = {}
    for _, result in ipairs(results) do
        jobs[#jobs + 1] = { job_name = result.job_name, job_label = result.job_label }
    end
    return jobs
end)
            ...
            for _, result in ipairs(results) do
                ...
                MySQL.Async.fetchAll('SELECT * FROM basjob_ranks WHERE job_name = @jobName', {
                    ...
                    if ranks and #ranks > 0 then
                        ...
                        exports.qbx_core:CreateJob(jobName, job, false)
                    end
                end)
            end
        else
...
lib.callback.register('jobcreator:getPublicMarkers', function()
    return MySQL.query.await('SELECT * FROM public_markers') or {}
end)
