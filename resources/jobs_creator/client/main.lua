...
RegisterNetEvent('jobcreator:publicMarkers', function()
    ...
    CreateThread(function()
        local jobs = lib.callback.await('jobcreator:getJobs', false) or {}
        if not jobs then
            print("No jobs found.")
            return
        end
...
function getPublicMarkers()
    CreateThread(function()
        local markers = lib.callback.await('jobcreator:getPublicMarkers', false) or {}
...
AddEventHandler('jobcreator:showJobs', function(jobs)
    ...
    for _, job in ipairs(jobs) do
        table.insert(jobOptions, {
            ...
            onSelect = function()
                CreateThread(function()
                    local ranks = lib.callback.await('jobcreator:getRanksForJob', false, job.name) or {}
                    local rankOptions = {}
