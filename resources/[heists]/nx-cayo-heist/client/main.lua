...
local function HackBlip(coords)
    hackBlip = AddBlipForCoord(coords.x, coords.y, coords.z)
...
local function OnHack1Done(success)
    if success then 
        ...
        if QBCore.Shared.Items[Config.Hack1Item] then TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Hack1Item], 'remove') end
        RemoveBlip(hackBlip)
...
local function OnHack3Done(success)
    if success then 
        ...
        if QBCore.Shared.Items[Config.Hack3Item] then TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Hack3Item], 'remove') end
        RemoveBlip(hackBlip)
...
local PlantBomb1 = function()
    ...
    Wait(100)
    local rotx, roty, rotz = GetEntityRotation(PlayerPedId())
...
RegisterNetEvent('nxte-cayo:client:startheist', function()
    ...
    local ped = PlayerPedId()
    SetEntityCoords(ped, 4818.67, -4309.54, 4.7, false, false, false, false)
...
RegisterNetEvent('nxte-anim:hack1', function()
    ...
    local posX, posY, posZ = (pos and pos.x) or 0, (pos and pos.y) or 0, (pos and pos.z) or 0
    local rot = GetEntityRotation(ped)
    local rotX, rotY, rotZ = (rot and rot.x) or 0, (rot and rot.y) or 0, (rot and rot.z) or 0
    ...
    -- props (ensure numbers for native calls)
    local phone = CreateObject(GetHashKey('prop_phone_ing'), posX, posY, posZ, true, true, false)
    local usb = CreateObject(GetHashKey('hei_prop_hst_usb_drive'), posX, posY, posZ, true, true, false)

    local netScene = NetworkCreateSynchronisedScene((animPos and animPos.x) or 0, (animPos and animPos.y) or 0, (animPos and animPos.z) or 0, (rotX or 0), (rotY or 0), (rotZ or 0), 2, false, false, 1065353216, 0, 1.3)
    ...
    local netScene2 = NetworkCreateSynchronisedScene((animPos2 and animPos2.x) or 0, (animPos2 and animPos2.y) or 0, (animPos2 and animPos2.z) or 0, (rotX or 0), (rotY or 0), (rotZ or 0), 2, false, true, 1065353216, 0, 1.3)
    ...
    local netScene3 = NetworkCreateSynchronisedScene((animPos3 and animPos3.x) or 0, (animPos3 and animPos3.y) or 0, (animPos3 and animPos3.z) or 0, (rotX or 0), (rotY or 0), (rotZ or 0), 2, false, false, 1065353216, 0, 1.3)
...
RegisterNetEvent('nxte-anim:hack2', function()
    ...
    FreezeEntityPosition(ped, true)
    local netScene = NetworkCreateSynchronisedScene((animPos and animPos.x) or 0, (animPos and animPos.y) or 0, (animPos and animPos.z) or 0, rotX, rotY, rotZ, 2, false, false, 1065353216, 0, 1.3)
    local bag = CreateObject(GetHashKey('hei_p_m_bag_var22_arm_s'), posX, posY, posZ, true, true, false)
    local laptop = CreateObject(GetHashKey('hei_prop_hst_laptop'), posX, posY, posZ, true, true, false)
    ...
    function() -- success
        ...
        NetworkStartSynchronisedScene(netScene3)
        NetworkStopSynchronisedScene(netScene3)
        if bag then DeleteObject(bag) end
        if laptop then DeleteObject(laptop) end
        FreezeEntityPosition(ped, false)
            ...
            if isHacked3 then
                if not Loot1 then 
                    ...
                    if random >= 2 then -- normal loot 
                        ...
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Loot1RareItem], 'add', chance)
                        TriggerServerEvent('nxte-cayo:server:setloot1', true)
                    end
                else
            ...
            if isHacked3 then
                if not Loot2 then 
                    ...
                    if random >= 2 then -- normal loot 
                        ...
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[Config.Loot2RareItem], 'add', chance)
                        TriggerServerEvent('nxte-cayo:server:setloot2', true)
                    end
                else
...
RegisterNetEvent('nxte-cayo:client:ResetHeist', function()
    if laptopprop then DeleteEntity(laptopprop) end
        ...
        for i=1, #netIds, 1 do
            ...
            if netId then
                ...
                if npc and DoesEntityExist(npc) then
                    ...
                    GiveWeaponToPed(npc, GetHashKey(Config.PedGun), 250, false, true)
...
Citizen.CreateThread(function()
    while true do 
        Citizen.Wait(1)
        if isActive then
            Citizen.Wait(math.max(0, (Config.StartTimer or 120) * 60000))
            RemoveBlip(hackBlip)
...
Citizen.CreateThread(function()
    while true do 
        ...
        if Loot1 and Loot2 and Loot3 then
            RemoveBlip(hackBlip)
            Citizen.Wait(math.max(0, (Config.FinishTimer or 60) * 60000))
            TriggerEvent('nxte-cayo:client:ResetHeist')
        end
    end
end)
