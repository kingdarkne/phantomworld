...
CreateThread(function()
    ...
    while true do
        -- Avoid showing HUD during character select/creator (tutorial session).
        if not NetworkIsInTutorialSession() and LocalPlayer.state.isLoggedIn then
            Wait(500)
...
RegisterNetEvent('hud:client:OnMoneyChange', function(type, amount, isMinus)
    -- Some money update emitters can pass `nil`/missing values.
    -- The HUD renders literal `undefined` if `amount` is not a number.
...
local function updateBaseplateHud(data)
    ...
    if shouldUpdate then
        SendNUIMessage ({
            ...
            showStreets = data[5],
            showPointer = data[6],
            -- NOTE: The NUI store uses the (typo) key `showDegress`.
            -- If we send `showDegrees` here, the HUD can end up with undefined state.
            showDegress = data[7],
        })
    end
end
...
local function getCrossroads(player)
    ...
    if updateTick - lastCrossroadUpdate > 5000 then
        ...
        local cross1 = GetStreetNameFromHashKey(street1)
        local cross2 = GetStreetNameFromHashKey(street2)
        -- Avoid sending nil into NUI; Svelte may render literal "undefined".
        if cross1 == nil then cross1 = '' end
        if cross2 == nil then cross2 = '' end
        lastCrossroadCheck = { cross1, cross2 }
    end
    -- Ensure we always return a 2-item array of strings.
    local cross1 = lastCrossroadCheck[1]
    local cross2 = lastCrossroadCheck[2]
    if cross1 == nil then cross1 = '' end
    if cross2 == nil then cross2 = '' end
