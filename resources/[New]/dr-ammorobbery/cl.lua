...
RegisterNetEvent('dr-ammorobbery:cl:clear')
AddEventHandler('dr-ammorobbery:cl:clear', function()
...
RegisterNetEvent('dr-ammorobbery:cl:start')
AddEventHandler('dr-ammorobbery:cl:start', function()

    local cops = lib.callback.await('dr-ammorobbery:sv:GetCops', false)
    local isCooldown = lib.callback.await('dr-ammorobbery:sv:coolc', false)
    if isCooldown then
        lib.notify({ description = 'In Cooldown', type = 'error' })
        return
    end
    if (tonumber(cops) or 0) < Config.CopAmount then
        lib.notify({ description = 'No Cops', type = 'error' })
        return
    end

    TriggerServerEvent("dr-ammorobbery:sv:coolout")
    TriggerServerEvent("dr-ammorobbery:sv:ClearSync")
...
function SetupContainers()
        ...
        exports["qb-target"]:AddCircleZone("opencontainers"..k, v.target, 1.0, {
            ...
            debugPoly=false
                ...
                options = {
                    {
                        action = function()
                            local required = tostring(Config.requiredItem or '')
                            local count = required ~= '' and (exports.ox_inventory:Search('count', required) or 0) or 0
                            if count > 0 then
    ...
    TriggerServerEvent("dr-ammorobbery:sv:synctarget")
...
function OpenContainer(index)
            ...
            AlertCops()
            -- Hook fenix-police: ammo robbery = 3 stars
            if GetResourceState('fenix-police') == 'started' then
                pcall(function()
                    exports['fenix-police']:ApplyWantedLevel(3)
                end)
            end
            ...
            TriggerServerEvent('dr-ammorobbery:sv:lockSync', index)
            ...
            TriggerServerEvent('dr-ammorobbery:sv:containerSync', GetEntityCoords(sceneObject), GetEntityRotation(sceneObject), index)
            TriggerServerEvent('dr-ammorobbery:sv:objectSync', NetworkGetNetworkIdFromEntity(sceneObject))
            TriggerServerEvent('dr-ammorobbery:sv:objectSync', NetworkGetNetworkIdFromEntity(lockObject))
...
RegisterNetEvent('dr-ammorobbery:cl:containerSync')
AddEventHandler('dr-ammorobbery:cl:containerSync', function(coords, rotation, index)
...
RegisterNetEvent('dr-ammorobbery:cl:lockSync')
AddEventHandler('dr-ammorobbery:cl:lockSync', function(index)
    Config['containers'][index]['lock']['taken'] = true
end)

RegisterNetEvent('dr-ammorobbery:cl:objectSync')
AddEventHandler('dr-ammorobbery:cl:objectSync', function(e)
...
RegisterNetEvent('dr-ammorobbery:cl:targetsync')
AddEventHandler('dr-ammorobbery:cl:targetsync', function()
