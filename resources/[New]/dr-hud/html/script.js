window.addEventListener('message', function (event) {
  ...
  if (data.action === 'updateHUD') {
    ...
    if (typeof data.fuel === 'number') {
      setText('fuel', Math.round(data.fuel) + '%');
      pulse('fuel');
    } else {
      setText('fuel', 'N/A');
    }
    ...
    if (carHud) {
      carHud.classList.toggle('is-hidden', !data.inVehicle);
    }

    if (data.inVehicle) {
      if (typeof data.speed === 'number') {
        setText('carSpeed', Math.round(data.speed).toString());
      }
      if (data.gear) {
        setText('carGear', String(data.gear));
      }
      if (typeof data.engineHealth === 'number') {
        setText('carEngine', Math.round(data.engineHealth) + '%');
      }
      if (typeof data.fuel === 'number') {
        setText('carFuel', Math.round(data.fuel) + '%');
      }
    }
  }
});
