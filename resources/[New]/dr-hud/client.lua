...
local function sendHudUpdate()
    ...
    if vehicle ~= 0 and DoesEntityExist(vehicle) then
        ...
        speed = GetEntitySpeed(vehicle) * 2.23694 -- Convert m/s to mph
        gear = GetVehicleCurrentGear(vehicle)
        if gear == 0 then gear = 'R'
        elseif gear == 1 then gear = 'N'
        else gear = tostring(gear - 1) end
        engineHealth = (GetVehicleEngineHealth(vehicle) / 1000) * 100
...
CreateThread(function()
    while true do
        if isPlayerLoggedIn() then
            -- Hide HUD when pause menu is active or NUI is focused
