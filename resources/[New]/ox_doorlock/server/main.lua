...
RegisterNetEvent('ox_doorlock:editDoorlock', function(id, data)
	local isAdmin = IsPlayerAceAllowed(source, 'command.doorlock')
	local isPsHousingDoor = data and data.name and data.name:match('^ps_mloproperty')
	if isAdmin or (not id and isPsHousingDoor) then
		if data then
...
-- ps-housing: Remove property doors when property is deleted (name format: ps_mloproperty*)
local function GetNameFromDoor(name)
...
RegisterNetEvent('ox_doorlock:RemoveDoorlock', function(name)
	if not name or not name:match('^ps_mloproperty') then return end
