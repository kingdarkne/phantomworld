...
lib.callback.register('dr-Drivingschool:payment', function(source)
    local src = source
    ...
    if bankamount >= amount then
        ...
        TriggerClientEvent('dr-Drivingschool:paymentSuccess', src)
    else
        exports.qbx_core:Notify(src, 'Not enough money', 'error')
    end
end)


lib.callback.register('dr-Drivingschool:server:GetLicense', function()
    local src = source
