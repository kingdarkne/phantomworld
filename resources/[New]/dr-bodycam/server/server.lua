...
lib.callback.register('dr-bodycam:servercb:getPedCoords', function(source, targetId)
...
lib.callback.register('dr-bodycam:servercb:getCarCoords', function(source, netId)
...
lib.callback.register('dr-bodycam:servercb:getNamESX', function(source)
    local xPlayer  = ESX.GetPlayerFromId(source)
    local Name = xPlayer.getName()
    if Name then return Name end
end)

RegisterNetEvent('dr-bodycam:server:toggleList',function(bool)
    local src = source
    if bool then 
    ...
    SetTimeout(1000, function()
        TriggerClientEvent('dr-bodycam:updatelisteffect',-1) 
    end)

end)

RegisterNetEvent('dr-bodycam:server:toggleListCars',function(bool,entityId,carPlate,carName,carClass)
    local src = source
    if bool then 
    ...
    SetTimeout(1000, function()
        TriggerClientEvent('dr-bodycam:updatelisteffectcar',-1) 
    end)
end)

RegisterNetEvent('dr-bodycam:server:ReqDeleteDecoyPed',function()
    if not Config.Dependency.UseAppearance then return end
    local src = source
    TriggerClientEvent('dr-bodycam:client:deleteDecoyPed',-1,src)
end)

RegisterNetEvent('dr-bodycam:server:ReqDecoyPed', function(cid, pedCoords)
    if not Config.Dependency.UseAppearance then return end
    local src = source
    local result
    local function handleDecoyPed(model, skin, pedCoords, src)
        ...
        if nPlayers then
            for i = 1, #nPlayers do
                local targetId = nPlayers[i].id
                -- Use current server id only (avoid oldId from playerJoining / invalid target 65536)
                if targetId and targetId > 0 and GetPlayerPing(targetId) >= 0 then
                    TriggerClientEvent('dr-bodycam:client:createDecoyPed', targetId, model, skin, pedCoords, src)
                end
            end
        end
    end
...
Citizen.CreateThread(function()
    if Config.Framework == 'qb' then
        exports.qbx_core:CreateUseableItem('bodycam', function(source, item)
            TriggerClientEvent('dr-bodycam:bodycamstatus',source)
        end)
        exports.qbx_core:CreateUseableItem('dashcam', function(source, item)
            TriggerClientEvent('dr-bodycam:toggleCarCam',source)
        end)
    else
        ESX.RegisterUsableItem('bodycam', function(playerId) 
            TriggerClientEvent('dr-bodycam:bodycamstatus', playerId) 
        end)
        ESX.RegisterUsableItem('dashcam', function(playerId) 
            TriggerClientEvent('dr-bodycam:toggleCarCam', playerId) 
        end)
    end
end)

RegisterNetEvent('dr-bodycam:server:logVideoDetails', function(videoUrl,streetName)
    local src = source
    local offName 
    local offJob  
    local offRank 
...
RegisterNetEvent('dr-bodycam:server:deleteVideoDB', function(videoUrl)
    local src = source
    ...
    MySQL.Async.execute('DELETE FROM spy_bodycam WHERE videolink = @videolink', {
        ...
        if rowsChanged > 0 then
            ...
            MySQL.Async.fetchAll('SELECT * FROM spy_bodycam WHERE job = @job ORDER BY id DESC', {
                ...
                TriggerClientEvent('dr-bodycam:client:refreshRecords', src, records, isBoss)
            end)    
        end
    end)
end)

RegisterNetEvent('dr-bodycam:server:showrecordingUI', function()
    local src = source
    local offJob   
    local jobKey
    local isBoss 
    ...
    MySQL.Async.fetchAll('SELECT * FROM spy_bodycam WHERE job = @job ORDER BY id DESC', {
        ...
        TriggerClientEvent('dr-bodycam:client:openRecords', src, records, offJob, isBoss)
    end)    
end)
...
lib.addCommand('recordcam', {
    ...
    if PlayerOnBodycam[src] then
        ...
        TriggerClientEvent('dr-bodycam:client:startRec',src,defwebhook,Upload.ServiceUsed)
    else
...
local fxManifestUrl = "https://raw.githubusercontent.com/Your-Spy/dr-bodycam/main/%5Bdr-bodycam%5D/dr-bodycam/fxmanifest.lua"
...
local function checkForUpdates()
    PerformHttpRequest(fxManifestUrl, function(statusCode, response, headers)
        print([[^4
╔───────────────────────────────────────────────────────────────────────╗
   ____  ____      _    ____ _   _ _        _     __     ______   _ 
 |  _ \|  _ \    / \  / ___| | | | |      / \    \ \   / / ___| / |
 | | | | |_) |  / _ \| |   | | | | |     / _ \    \ \ / /|___ \ | |
 | |_| |  _ <  / ___ \ |___| |_| | |___ / ___ \    \ V /  ___) || |
 |____/|_| \_\/_/   \_\____|\___/|_____/_/   \_\    \_/  |____(_)_|
...
╚───────────────────────────────────────────────────────────────────────╝
        ...
        if statusCode == 200 then
            ...
            print("^2your running the latest update: " .. statusCode)
        end        
