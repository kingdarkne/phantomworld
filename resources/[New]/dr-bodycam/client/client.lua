...
AddEventHandler('onResourceStart', function(resourceName)
	...
	local pd = GetPlayerDataCore()
	PlayerJob = pd and pd.job or nil
end)
...
RegisterNetEvent('dr-bodycam:startWatchingDashcam',function(netId)
    local targetCoords = lib.callback.await('dr-bodycam:servercb:getCarCoords', false, netId)
    ...
    if Config.Framework == 'qb' then
        TriggerServerEvent('dr-bodycam:server:ReqDecoyPed',GetPlayerDataCore().citizenid,goBackCoords)
    elseif Config.Framework == 'esx' then 
        TriggerServerEvent('dr-bodycam:server:ReqDecoyPed',GetPlayerDataCore().identifier,goBackCoords)
    end
    Wait(500)
...
RegisterNetEvent('dr-bodycam:startWatching',function(targetId)
    local ownId = GetPlayerServerId(PlayerId())
    if targetId == ownId then return TriggerEvent('dr-bodycam:startSelfWatching',targetId) end
    local targetCoords = lib.callback.await('dr-bodycam:servercb:getPedCoords', false, targetId)
...
RegisterNetEvent('dr-bodycam:startSelfWatching',function(targetId)
    targetPed = cache.ped
    targetPedId = targetId
    LocalPlayer.state:set("inv_busy", true, true) TriggerEvent('inventory:client:busy:status', true) TriggerEvent('canUseInventoryAndHotbar:toggle', false)
    DoScreenFadeOut(1000)
...
RegisterNetEvent('dr-bodycam:bodycamstatus',function()
    ...
    if Config.Dependency.UseProgress == 'ox' then
        if lib.progressBar({
            ...
            TriggerServerEvent('dr-bodycam:server:toggleList',bcamstate)
...
RegisterNetEvent('dr-bodycam:toggleCarCam', function()
    local isJobUse = CheckAllowedJob()
    if not isJobUse then return NotifyPlayer('You are not authorized!', 'error', 2500) end
...
RegisterNetEvent('dr-bodycam:updatelisteffect',function()
...
RegisterNetEvent('dr-bodycam:updatelisteffectcar',function()
    if PlyInCarCam then
...
RegisterNetEvent('dr-bodycam:openActiveMenu', function(locId)
    local optionsMenu = {}
    ...
    if Config.Dependency.UseMenu == 'ox' then 
        optionsMenu[#optionsMenu+1] = {
            ...
            icon = 'user',
            onSelect = function()
                TriggerEvent('dr-bodycam:openActiveMenuJob', locId)
            end
        }
        optionsMenu[#optionsMenu+1] = {
            ...
            icon = 'car',
            onSelect = function()
                TriggerEvent('dr-bodycam:openActiveMenuCars', locId)
            end
        }
...
RegisterNetEvent('dr-bodycam:openActiveMenuJob', function(locId)
    local optionsMenu = {}
    if Config.Dependency.UseMenu == 'qb' then
        ...
        optionsMenu[#optionsMenu+1] = { 
            ...
            params = {
                isAction = true,
                event = function()
                    TriggerEvent('dr-bodycam:openActiveMenu',locId)
                end
            } 
        }
            ...
            if isLocFilterTrue(locId,v.jobkey) then 
                if Config.Dependency.UseMenu == 'ox' then 
                    optionsMenu[#optionsMenu+1] = {
                        ...
                        icon = 'video',
                        onSelect = function()
                            TriggerEvent('dr-bodycam:startWatching',k)
    ...
    elseif Config.Dependency.UseMenu == 'esx' then
        ESX.OpenContext("right" , optionsMenu,
        function(menu,element)
            if element.name == 'back' then TriggerEvent('dr-bodycam:openActiveMenu',locId) return end
            TriggerEvent('dr-bodycam:startWatching',element.name)
            local coord = GetEntityCoords(cache.ped)
            goBackCoords = vector4(coord.x, coord.y, coord.z -1 , GetEntityHeading(cache.ped))
            SetTimeout(2000, function()
...
RegisterNetEvent('dr-bodycam:openActiveMenuCars', function(locId)
    local optionsMenu = {}
    if Config.Dependency.UseMenu == 'qb' then
...
RegisterNetEvent('dr-bodycam:client:createDecoyPed', function(model, data, pVec4, plyId)
...
RegisterNetEvent('dr-bodycam:client:deleteDecoyPed',function(plyId)
    if not Config.Dependency.UseAppearance then return end
...
RegisterNetEvent('dr-bodycam:client:startRec',function(webhook,serviceUsed)
    SendNUIMessage({
...
RegisterNetEvent('dr-bodycam:client:openRecords',function(records,jobTitle,isBoss)
...
RegisterNetEvent('dr-bodycam:client:refreshRecords',function(records,isBoss)
    SendNUIMessage({
        action = "refreshrec",
        recordData = records,
        isBoss = isBoss
...
RegisterNUICallback('videoLog', function(data, cb)
    if data then 
        ...
        TriggerServerEvent('dr-bodycam:server:logVideoDetails', videoUrl,GetStreetNameFromHashKey(s1))
    end
end)
...
RegisterNUICallback('deleteVideo', function(data, cb)
    if data then 
        local videoUrl = data.vidurl
        TriggerServerEvent('dr-bodycam:server:deleteVideoDB', videoUrl)
    end
end)
...
function StartDashCamWatch(k,plate,carname)
    local targetCoords = lib.callback.await('dr-bodycam:servercb:getCarCoords', false, k)
    if not targetCoords then NotifyPlayer('Car not found!', 'error', 2500) return end    
    TriggerEvent('dr-bodycam:startWatchingDashcam',k)
    local coord = GetEntityCoords(cache.ped)
    goBackCoords = vector4(coord.x, coord.y, coord.z -1 , GetEntityHeading(cache.ped))
    SetTimeout(2000, function()
...
function ToggleCarCam(netId, veh)
    if not GlobalState.CarsOnBodycam[netId] then 
        ...
        TriggerServerEvent('dr-bodycam:server:toggleListCars', true, netId, carPlate, carName,carClass)
    else
        TriggerServerEvent('dr-bodycam:server:toggleListCars', false, netId)
    end
end
...
function QuitBodyCam()
    ...
    TriggerServerEvent('dr-bodycam:server:ReqDeleteDecoyPed')
...
function BodyOverlay(bool)
    if bool then
        if Config.Framework == 'qb' then
            ...
            local bodyId = GetPlayerServerId(PlayerId())
            local playerName = lib.callback.await('dr-bodycam:servercb:getNamESX', true)
            local randomBodyNum = "BODY "..bodyId.. " | ".."X"..tostring(math.random(100000, 999999)) .. "N"
...
function CheckForItem()
    while bcamstate do 
        ...
        if not hasItem then
            TriggerServerEvent('dr-bodycam:server:toggleList',false)
            BodyOverlay(false)
            toggleProp(false)
            bcamstate = false
            SendNUIMessage({action = "cancel_rec_force"})
...
function GetPlayerDataCore()
    if Config.Framework == 'qb'  then
        ...
        elseif GetResourceState('qbx_core') == 'started' then
            pd = exports.qbx_core:GetPlayerData()
        end
        if pd then
            return pd
        end
