...
AddEventHandler("utk_fh:outcome", function(oc, arg)
    ...
    if oc then
        ...
        -- Hook fenix-police: bank robberies = 4 stars (more serious than store robbery)
        if GetResourceState('fenix-police') == 'started' then
            pcall(function()
                exports['fenix-police']:ApplyWantedLevel(4)
            end)
        end
    elseif not oc then
        QBCore.Functions.Notify(arg, "error")
    end
end)
            ...
            while true do
                ...
                if dst < 40 then
                    if not LootCheck[name].Loot1 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley1.x, data.trolley1.y, data.trolley1.z+1, "~g~E~w~ - Loot the Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot2 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley2.x, data.trolley2.y, data.trolley2.z+1, "~g~E~w~ - Loot the Cash", 0.40)
                    ...
                    if not LootCheck[name].Loot3 then
                        ...
                        if dst1 < 5 then
                            DrawText3D(data.trolley3.x, data.trolley3.y, data.trolley3.z+1, "~g~E~w~ - Loot the Cash", 0.40)
                            if dst1 < 1 and IsControlJustReleased(0, 38) then
...
AddEventHandler("utk_fh:policenotify", function(name)
    ...
    if PlayerData.job.name == "police" then
        QBCore.Functions.Notify("A bank's alarms are triggered!", 'error', 10000)
            ...
            if PlayerData.job.name == "police" and not dooruse then
                ...
                for k, v in pairs(Doors) do
                    for i = 1, 2, 1 do
                        ...
                        if dst <= 4.0 then
                            if v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "~g~E~w~ - Unlock the door", 0.40)
                            elseif not v[i].locked then
                                DrawText3D(v[i].txtloc[1], v[i].txtloc[2], v[i].txtloc[3], "~g~E~w~ - Lock the door", 0.40)
                            end
...
function SecondDoor(data, key)
    ...
    QBCore.Functions.Notify("Access complete!", 'success')
    PlaySoundFrontend(-1, "ATM_WINDOW", "HUD_FRONTEND_DEFAULT_SOUNDSET")
...
function Process(ms, text)
    exports['taskbar']:taskBar(ms, text)
    Citizen.Wait(ms)
end
...
Citizen.CreateThread(function()
    while true do
        if not done then
            ...
            if dst < 4 then
                DrawText3D(UTK.Banks[currentname].doors.secondloc.x, UTK.Banks[currentname].doors.secondloc.y, UTK.Banks[currentname].doors.secondloc.z, "~g~E~w~ - Use Secure ID Card", 0.40)
                if dst < 0.75 and IsControlJustReleased(0 , 38) then
                    QBCore.Functions.TriggerCallback("utk_fh:checkSecond", function(result)
                        if result then
                            done = true
...
Citizen.CreateThread(function()
    ...
    QBCore.Functions.TriggerCallback("utk_fh:getBanks", function(bank, door)
        UTK.Banks = bank
        Doors = door
    end)
    --Citizen.Wait(1000)
