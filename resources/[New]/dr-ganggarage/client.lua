...
AddEventHandler('onResourceStart', function(r) if GetCurrentResourceName() ~= r then return end
	QBCore.Functions.GetPlayerData(function(PlayerData) PlayerGang = PlayerData.gang end)
end)
...
CreateThread(function()
	for k, v in pairs(Config.Locations) do
		if v.garage then
			...
			Targets["GangGarage: "..k] =
			exports['qb-target']:AddBoxZone("GangGarage: "..k, vector3(out.x, out.y, out.z-1.03), 0.8, 0.5, { name="GangGarage: "..k, heading = out.w+180.0, debugPoly=Config.Debug, minZ=out.z-1.05, maxZ=out.z+0.80 },
				{ options = { { event = "pwd-garagegang:client:Garage:Menu", icon = "fas fa-clipboard", label = "gang Vehicles", gang = v.gang, coords = v.garage.spawn, list = v.garage.list, prop = Parking[#Parking] }, },
				distance = 2.0 })
		end
	end
end)
...
RegisterNetEvent('pwd-garagegang:client:Garage:Menu', function(data)
	if not IsPedInAnyVehicle(PlayerPedId()) then
		lookEnt(data.prop)
		loadAnimDict("amb@prop_human_atm@male@enter")
		TaskPlayAnim(PlayerPedId(), 'amb@prop_human_atm@male@enter', "enter", 1.0,-1.0, 1500, 1, 1, true, true, true)
	end
	local vehicleMenu = { { icon = "fas fa-car-tunnel", header = PlayerGang.label.." ".."Gang Garage", isMenuHeader = true } }
	vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = "Close", params = { event = "pwd-garagegang:client:Menu:Close" } }
	if currentVeh.out and DoesEntityExist(currentVeh.current) then
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-clipboard-list", header = "Vehicle out of Garage",
										...
										params = { event = "pwd-garagegang:client:Garage:Blip", }, }
		vehicleMenu[#vehicleMenu+1] = { icon = "fas fa-car-burst", header = "Remove Vehicle", params = { event = "pwd-garagegang:client:RemSpawn" } }
	else
		...
		for k, v in pairsByKeys(data.list) do
			...
			if v.grade then if v.grade <= PlayerGang.grade.level then showButton = true end end
			if v.rank then for _, b in pairs(v.rank) do if b == PlayerGang.grade.level then showButton = true end end end
			...
			if showButton == true then
				...
				vehicleMenu[#vehicleMenu+1] = { icon = seticon, header = (data.list[k].CustomName or searchCar(k)), params = { event = "pwd-garagegang:client:SpawnList", args = { spawnName = spawnName, coords = data.coords, list = v } } }
			end
		end
	end
	showButton = nil
...
RegisterNetEvent("pwd-garagegang:client:SpawnList", function(data)
	...
	if oldveh ~= 0 then
		...
		QBCore.Functions.SpawnVehicle(data.spawnName, function(veh)
			...
			SetVehicleNumberPlateText(veh, string.sub(PlayerGang.label, 1, 5)..tostring(math.random(100, 999)))
...
RegisterNetEvent("pwd-garagegang:client:RemSpawn", function(data)
...
RegisterNetEvent("pwd-garagegang:client:Garage:Blip", function(data)
	triggerNotify(nil, "Gang Vehicle Marked on Map")
	if markerOn then markerOn = not markerOn end
	markerOn = true
	if not DoesBlipExist(garageBlip) then
		garageBlip = makeBlip({ coords = GetEntityCoords(currentVeh.current), sprite = 85, col = 8, name = "Gang Vehicle" })
