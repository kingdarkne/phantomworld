...
$(document).ready(() => {
  ...
  $(".profile-items").on("click", ".profile-item", async function () {
    ...
    // Add wanted tag (from dr-wanted) at the top of tags list, if any
    if (result.wanted_level && result.wanted_level > 0) {
      const wantedText = `Wanted: ${result.wanted_level} \u2B50 (${result.wanted_points || 0} pts)`;
      tagsHTML = `<div class="white-tag">${wantedText}</div>`;
    }
